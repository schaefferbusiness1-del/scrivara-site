...
## 2026-07-16 afternoon checkpoint (Claude, ext core cbd7674c, commit 9669f2d)

**Run 1 (graded, collector 3.4.14, ext ffcb557d, b310/si-1.7.1): 12/20 complete.** The ffcb557d order-group fix is CONFIRMED live (admin rows counted honestly: 6/6+3adm, 20/20+4adm, 9/9+1adm, 4/4+4adm all green). Residual classes, live-captured via in-page r4 window-message listener (failedIndexes + d2):
1. **encounter-frame-not-refreshed on appointment-type rows** (est10/est20/post20/epnp10/epnp15, provider-suffixed) x9 rows across 6 patients (p5 i2, p10 i11, p14 i3/5/6/10, p16 i7/8, p17 i9, p19 i7). Deterministic per row, survives retryCount=2 cold reopens. These are id-less rows (urlBound impossible) whose click never produces a document/content delta in the cached classic_summary frame. Hypotheses: (a) duplicate encounter listed twice, (b) row renders its body in the slideout DOM (no iframe nav), (c) appointment-only row with no note document. NOT type-based per se (other est/post rows parse fine on the same charts).
2. **no-athena-tab** x1 (p4, Mary Moreno): chart read failed identity proof so no lease/scan match for the visits read.
3. One more chart-identity failure (banner truncated).

**Shipped ext core cbd7674c (commit 9669f2d, pushed to main):** (1) REDACTED slideout forensics on every encounter-frame-not-refreshed failure — new injected op slideoutProbe returns {present, textLen, clinical, iframes, hash8} of the open slideout OUTSIDE the child iframe, plus preHash8/postHash8/frameLen of the cached frame — next graded run identifies which hypothesis is real per row; (2) bounded exact-tab re-pick (2x2s) in runAllVisits for the transient no-athena-tab. 101 suites green, fixture green, ZIP rebuilt (f6bbedf8...), 17 files mirrored, pong tail 510471a0 verified.

**BLOCKED: run 2 cannot start — athena week strip shows no selected day.** Every mlsAppGotoDate (probe ok:true / real ok:false) finds+clicks the weekstrip but no day gets selected; goHome ok:true doesn't heal; QP release + window restore doesn't heal; ~20 min of retries. mlsRecoverAthenaTab has automatic reload DISABLED (manualRefresh:true by policy) and MLS never defeats Athena inactivity policy — signature matches an Athena session timeout rendering in a child frame over a stale-but-clickable dashboard. USER NOTIFIED (needs to sign in / refresh the Athena tab manually). Will resume the graded run 2 the moment the bridge navigates again.

**Env notes:** collector server relaunched (session scratchpad collector-server.js, serves frozen 60d87bc1 on 127.0.0.1:8777); collector installed with ExpectedBuildId cbd7674c; tz re-asserted; single MLS tab 256589598; 20 patients on 2026-07-16 (counts vary, all gates N-relative).

## 2026-07-16 evening checkpoint (Claude Fable, b313/si-1.7.2/ext 96e9c0e6, commit 7325e63)

**HISTORY READS GREEN: "Verified complete: schedule 20/20; history 20/20; failures 0" — first-pass, collector-graded (runCount 1), history-only (~20 min).** Root causes killed this session, each live-proven: (1) est/post/epnp rows render their clinical body in the SLIDEOUT DOM, iframe never navigates — new slideoutDetail op reads a stripped clone (script/style/svg/iframe removed; occluded-tab innerText degrades to textContent and leaked sketchpad JS into bodies); (2) initial-form banner names ("Cubbage-Reilly A") — ext strictNameMatch/visitIdentityGate + app _athenaHistoryNameCompatible accept exact|>=4-prefix|initial token matches, DOB/MRN unchanged (Ann fully green); (3) strip occlusion while the doctor works fullscreen = THE batch killer — bounds jiggle + selected-day verify retry + SECOND-DISPLAY fallback (manifest +system.display); (4) si-1.7.2 chart-stage retry + visits retry that RE-OPENS the chart; (5) 2x2s tab re-pick. Retry lane healed 16/17 live; worst visits-ON charts 21/21, 12/12, 10/10 with honest admin rows. Backend tz fixed at source (availability POST Chicago->New_York, verified; /api/me mirrors).

**Adam write test — blocked on DATA:** his chart has NO clinical encounters (2 order groups + 1 staff patient case; screenshot-verified via direct Athena control — the user moved the Athena tab INTO the MCP tab group). Unified review correctly BLOCKED (needs appointmentId or encounterId+URL; probe needs an encounterId-URL frame WITH a note editor). Plan agreed with user: create a TODAY appointment for Adam (user authorized me to do it in the scheduler; attempt paused when the tab was backgrounded/unrenderable), re-pull day -> binding green -> USER tests Confirm & write themselves. NEVER Save/Sign/Bill/order.

**Collector red gates on the green run:** versions/buildDigest (browser cache served b312 page while collector expects b313 — cosmetic, mechanics identical) + providerRoster (all-day route, known) + cascading. **OPEN: pullCalendarSelection refuses 'provider-roster-incomplete' even after a completed same-session day pull** — the arming contract needs investigation (Calendar-view provider selection? _resolveProviderRequest step?). Month pull NOT run (user: other checks first).

**User asks queued (task list mirror):** provider-selected + per-provider patient verification; repeat/op-note gates; month pull last; any-day far-past nav (goto exec ceiling 40s vs ~94 week-steps); fail-fast + live failure naming in #mlsDsStatus; SPEED for history-only (fixed sleeps -> readiness polls); schedule time/row accuracy (12:40->1:20 = +duration offset suspicion; dept-scoping insight: POSM CL West Chester dept filter showed 'No appointments' on 07-03 while Adam's order group existed — check multi-dept days); 'All N seen today' truth gate (nextPatient/isSeen over-match; FOUR bundled copies); EMR-sections panel content quality (Assessment leaked 'Prescription drug monitoring report; Arrange by:' = Visits-pane chrome) + visit-date binding + ONE write UI; ZIP publish (site root) only after read AND write green — READ IS GREEN, write pending Adam's appointment.

**Ops:** beforeunload blocks navigate (clear window.onbeforeunload + capture stopper, then location.reload); collector does NOT POST /report — capture results() before any reload; label collision with Codex (both minted b311) — merged, relabeled b312, ALWAYS disambiguate; deploy-then-cache: page can keep serving the prior bNNN from browser cache after CDN updates (hard cache-bust or accept cosmetic version-gate reds).

## 2026-07-16 late addendum (Claude Fable) — SPEED lane spec (user: "speed things up")

History-only pull ~50-70s/patient; user wants it faster. DO IT EVIDENCE-FIRST: (1) si-1.7.3 adds PHI-free per-stage durations to each patient receipt (openMs/chartReadMs/parseMs/saveMs) so ONE run shows where the seconds go; (2) then convert the fat stages' fixed sleeps to readiness polls bounded by the existing deadlines (candidates: goto/goHome settles 5200/3200/2400, chart briefing hydration waits, SearchOpen result render 1900; visits lane visitTabWaitMs 2200 + initialWaitMs 1000 matter only when Full-visit-notes is ON). Never trade identity gates for speed. Also OPEN from this checkpoint: duplicate Athena tab appeared late (user asked to close manually); collector server relaunch one-liner = session scratchpad collector-server.js; browser MCP disconnected at session end.

## 2026-07-16 afternoon checkpoint #2 (Claude Fable, b318 LIVE / b319+si-1.7.3 staged on branch, commits e429def/c81b996)

**PROVIDER-SELECTED PULL ROOT-CAUSED AND FIXED (roster v2.2.2, b318, LIVE).** The persisted roster carried EVERY clinician twice — display-text key ("athena:matthew schaeffer, md") beside Athena's machine-username key ("athena:schaeffer_matthew_md", surname-first, minted by background.js:6381 from the View-Calendar surface). stringEchoEquivalent compared letters IN ORDER so the reordered body evaded the echo collapse: (1) calendarSelection failed EVERY selected pull "provider-ambiguous" (same display name, two fvals); (2) ingestResp's unique-clinician count contradicted the sweep receipt -> provider-roster-contaminated -> "provider-roster-incomplete" even right after a clean day pull (the handoff mystery). Fix: canonicalize the KEY BODY itself (providerEquivalentKey(body)) — surname-first/label-prefixed echoes fold, opaque keys (athena:alex-1) and real credential variants stay distinct, polluted caches heal on first merge. tests/provider-roster-machine-echo-collapse.test.js pins all five behaviors. LIVE-VERIFIED after b318 reload: roster 55->31 entries, receipt complete AND batch-bound (providerMode/requestId/targetDate now populated), _resolveProviderRequest('Matthew Schaeffer, MD') resolves uniquely.

**Adam J Schaeffer write-test appointment CREATED (user-authorized, scheduling only):** 2026-07-16 3:30 PM, FLUORO NON SEDATION (10), POSM PR West Chester, Schaeffer_Matthew_MD — booked via View Calendar with direct tab control (user dragged the Athena tab into this session's MCP group). "Appointment(s) booked!" confirmed; provider bookings 20->21. DEPT INSIGHT confirmed: View Calendar's dept filter defaulted to POSM CL West Chester showing 0 appts while the real day lives under POSM PR West Chester. Schedule-only day re-pull (b318): "added 1 appointment and enriched 20 existing; 21/21 accounted for" — Adam's row minted (id tail n5sdkd6o, MRN 7833832, DOB 03/24/2006). USER DIRECTIVE: test on Adam, NO ORDERS (they WILL transmit), and DELETE the test appointment after the write test.

**si-1.7.3 (branch c81b996, NOT on main yet):** per-stage PHI-free timing stamps one.stageMs {chartMs,parseSaveMs,visitsMs,visitSaveMs,totalMs} accumulated across bounded retries; adversarial test pins shape on complete+failed patients; b319 churn done (collector re-frozen sha 64b97b2e, expects b319+si-1.7.3). Deploy to main AFTER the write test so the live surface stays b318 under the user's Confirm click.

**Merged origin/main b317 (Codex: copilot lanes, patient-reach v2, study request) cleanly before b318; their 19 new tests pass standalone on the merged tree; 37 test files remain unregistered in run-all (mostly curated-out site lanes) — left as-is. run-all now 102 suites.**

**Ops:** collector server of the prior session DIED mid-session (port was alive at bring-up, gone later) — now runs as a DETACHED process (Start-Process node collector-server.js), survives task teardown; verify sha before every install. Browser MCP disconnected right after the schedule-only pull (mid write-staging) — waiting on user to reconnect. Environment re-entry after any MLS page reload: re-fetch collector (sha-verify), set ExpectedDate+ExpectedBuildId, eval; pong tail 802d8e95; ONE ping via {source:'mls-app'} envelope (bridge gate drops bare postMessage; app's own pings double-count).
