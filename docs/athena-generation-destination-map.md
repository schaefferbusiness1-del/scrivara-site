...
`Procedure Documentation` was **not** live-verified as an exact editable destination. The extension now has a source-inspected, synthetic-tested supervised placement candidate for one already-open exact editor, but it does not create or select a procedure template and it must not be described as live-verified until the controlled proof below succeeds.
...
| Completed procedure or operative note | Exact already-open Physical Exam > Procedure Documentation editor | Supervised placement only; no template creation, save, sign, or order action | Require immutable `Procedure` identity, exactly one empty Procedure Documentation editor, exact payload bytes, active encounter identity, and exact readback. Missing, duplicate, ambiguous, or nonempty targets fail closed; this route never falls back to Physical Exam or the generic encounter note. Synthetic-tested only; live proof remains required. |
...
| Generated artifact | Athena destination | Allowed mode | Validation and proof requirement |
|---|---|---|---|
| Procedure or operative note when Procedure Documentation is absent, duplicated, ambiguous, or already contains different text | Physical Exam > Procedure Documentation | Manual copy/export only | The supervised lane refuses rather than creating a template, overwriting existing text, flattening into Physical Exam prose, or falling back to the generic encounter note. Open and verify the intended destination manually; live proof is still required before calling any path end-to-end verified. |
