...
"/* ---- nav rail ---- */",
"/* height:100vh, NOT top:0+bottom:0 stretch: the rail mounts INSIDE",
"   header#appHeader, whose backdrop-filter makes it the CONTAINING BLOCK for",
"   fixed descendants - so the stretch resolved to the header's ~87px box, the",
"   nav list collapsed to 0 and the burger opened an EMPTY menu (measured live",
"   2026-08-20; same trap the op-notes Home button hit). Viewport units resolve",
"   against the viewport regardless of containing block. Reparenting to body",
"   would instead break the two-id hide selectors (#appHeader #mlsRdNav) in",
"   feat_mls_offtab_navhide.js and feat_mls_simpleview_global.js. */",
"#mlsRdNav{ position:fixed; left:0; top:0; height:100vh; width:var(--rail-w); z-index:65; background:var(--bg); border-right:1px solid var(--line); display:flex; flex-direction:column; padding:14px 12px 12px; overflow:hidden; }",
