...
(function () {
  ...
  function apptDob(a) { var d = clean(a.dob); if (d) return d; var key = nameKey(a.name); var dm = dobMap(); if (key && dm[key]) return clean(dm[key]); return safe(function () { var ps = (isFn(window.getPatients) && window.getPatients()) || jget('patients') || []; for (var i = 0; i < ps.length; i++) { if (nameKey(ps[i].name) === key && clean(ps[i].dob)) return clean(ps[i].dob); } return ''; }, '') || ''; }
  function matchedAppts(doctorName) { var T = tags(); var sn = surname(doctorName); var ap = safe(function () { return window._calAppts || []; }, []) || []; var out = [], seen = {}; for (var i = 0; i < ap.length; i++) { var a = ap[i]; if (!a) continue; var pid = a.patient_external_id || a.patientId || a.pid; var tag = pid != null ? T[pid] : null; if (!tag) continue; if (sn && clean(tag).toLowerCase() !== sn) continue; var nm = clean(a.name); if (!isRealName(nm)) continue; var dob = apptDob(a); if (!dob) continue; var t = hhmm(a), dt = apptDate(a), k = nm.toLowerCase() + '|' + dt + '|' + t; if (seen[k]) continue; seen[k] = 1; out.push({ name: nm, time: t, date: dt, dob: dob, reason: clean(a.reason || ''), start_at: a.start_at }); } out.sort(function (x, y) { return (S(x.date) + 'T' + S(x.time)).localeCompare(S(y.date) + 'T' + S(y.time)); }); return out; }
  function allTaggedAppts() { var T = tags(); var ap = safe(function () { return window._calAppts || []; }, []) || []; var out = [], seen = {}; for (var i = 0; i < ap.length; i++) { var a = ap[i]; if (!a) continue; var pid = a.patient_external_id || a.patientId || a.pid; if (pid == null || !T[pid]) continue; var nm = clean(a.name); if (!isRealName(nm)) continue; var dob = apptDob(a); if (!dob) continue; var t = hhmm(a), dt = apptDate(a), k = nm.toLowerCase() + '|' + dt + '|' + t; if (seen[k]) continue; seen[k] = 1; out.push({ name: nm, time: t, date: dt, dob: dob, reason: clean(a.reason || ''), start_at: a.start_at }); } out.sort(function (x, y) { return (S(x.date) + 'T' + S(x.time)).localeCompare(S(y.date) + 'T' + S(y.time)); }); return out; }
  function currentList() { var doc = selectedDoctor(); return doc.all ? allTaggedAppts() : matchedAppts(doc.name); }
  function fixChip() { safe(function () { var rn = realName(); if (!rn) return; var wrong = scrapedName(); if (wrong && wrong !== rn) { var sp = document.querySelectorAll('span,div,small,b,strong'); for (var i = 0; i < sp.length; i++) { var el = sp[i]; if (el.children.length) continue; if (el.closest && el.closest('select,option,#mlsCalRoster')) continue; if (clean(el.textContent) === wrong) { if (el.getAttribute('data-ptfix-orig') == null) el.setAttribute('data-ptfix-orig', el.textContent); el.textContent = rn; } } } var roster = document.getElementById('mlsCalRoster'); if (roster && wrong && wrong !== rn) { var chips = roster.querySelectorAll('.mlsRosChip'); var hasReal = false, k; for (k = 0; k < chips.length; k++) if (clean(chips[k].textContent) === rn) hasReal = true; if (hasReal) { for (k = 0; k < chips.length; k++) { if (clean(chips[k].textContent) === wrong) { if (chips[k].getAttribute('data-ptfix-hide') == null) { chips[k].setAttribute('data-ptfix-hide', '1'); chips[k].style.display = 'none'; } } } } } }); }
