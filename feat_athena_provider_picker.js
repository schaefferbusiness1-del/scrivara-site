...
(function () {
  ...
  function start() { rerender(); safe(function () { _obs = new MutationObserver(function (records) { safe(function () { /* Ignore batches this module caused itself (its own box, or the name spans it just rewrote) — reacting to them re-scanned the whole document at the throttle rate and fed back into more mutations (intermittent freeze amplifier). */ var external = false; for (var i = 0; i < records.length; i++) { var t = records[i] && records[i].target; var el = t && (t.nodeType === 1 ? t : t.parentElement); if (!el) { external = true; break; } if (el.closest && (el.closest('#mlsPtfBox') || (el.getAttribute && el.getAttribute('data-ptfix-orig') != null))) continue; external = true; break; } if (!external) return; fixChip(); if (document.getElementById('heroToday') && !document.getElementById('mlsPtfBox')) renderWhosNext(); }); }); _obs.observe(document.documentElement, { childList: true, subtree: true }); }); _poll = setInterval(tick, 1500); [300, 900, 2000, 4000].forEach(function (ms) { setTimeout(function () { safe(rerender); }, ms); }); }
