...
const loadingPlan = prepare('feat_mls_loading_calm.js', 'utf8', [
  [
    ...
    ' * MLS shared job-progress store (lb-2.1.2)'
  ],
  [
    "  var VERSION = 'lb-2.1.0';",
    "  var VERSION = 'lb-2.1.2';"
  ],
  [
  ...
  [
    ...
    "    if (keyIndex[j.key] === id) delete keyIndex[j.key];\n    clearDeadline(id);\n    removeManual(id);\n    if (status === 'completed' || status === 'canceled' || typeof retryFns[id] !== 'function') {\n      delete retryFns[id]; delete cancelFns[id];\n    }\n    pruneTerminalJobs(id);\n    persist(); emit(j); sync();"
  ],
  [
...
const connectPlan = prepare('mls-connect.js', 'latin1', [
  [
    ...
    "var A='feat_mls_loading_calm.js',V='lb-2.1.2'"
  ],
  [
    "s.src=A+'?v=20260719lb204'",
    "s.src=A+'?v=20260729lb212a1'"
  ],
  [
    '/* lb-2.1.0 version-aware headless job store; retires b431 floating loading owner/tag before reload. */',
    '/* lb-2.1.2 version-aware headless job store; 2026-07-29 retires the stale floating loading owner/tag before reload. */'
  ]
]);
...
const sharedTestPlan = prepare('tests/shared-progress-runtime.test.js', 'utf8', [
  ...
  [
    ...
    "assert.strictEqual(api.version, 'lb-2.1.2');"
  ],
  [
...
const immutablePlan = prepare('tests/immutable-satellite-loader-cache-contract.test.js', 'utf8', [[
  ...
  "  ['feat_mls_loading_calm.js', '20260729lb212a1', '20260719lb204'],"
]]);
...
const progressTestPlan = prepare('tests/progress-stages-runtime.test.js', 'utf8', [
  [
    ...
    '/* ps-1.2.0 named-stage progress wiring: loads the REAL lb-2.1.2 owner plus the'
  ],
  [
  ...
  [
    ...
    "context.addEventListener = function (type, fn) {\n  if (type === 'message') { messageListeners.push(fn); return; }\n  (windowListeners[type] = windowListeners[type] || []).push(fn);\n};\ncontext.removeEventListener = function (type, fn) {\n  const list = type === 'message' ? messageListeners : (windowListeners[type] || []);\n  const at = list.indexOf(fn); if (at >= 0) list.splice(at, 1);\n};\ncontext.dispatchEvent = function (event) { (windowListeners[event.type] || []).slice().forEach(fn => fn(event)); };"
  ],
  [
    "assert(lb && lb.installed && lb.version === 'lb-2.1.0', 'shared lb owner missing');",
    "assert(lb && lb.installed && lb.version === 'lb-2.1.2', 'shared lb owner missing');"
  ],
  [
    "line.includes(\"var A='feat_mls_loading_calm.js',V='lb-2.1.0'\")",
    "line.includes(\"var A='feat_mls_loading_calm.js',V='lb-2.1.2'\")"
  ],
  [
    "lbLoader.includes(\"s.src=A+'?v=20260719lb204'\")",
    "lbLoader.includes(\"s.src=A+'?v=20260729lb212a1'\")"
  ],
  [
    ...
    "assert(psSource.includes('if (pullWatch.iv) clearInterval(pullWatch.iv)'), 'revert leaks the pull-stamp watcher interval');\n\n/* 2026-07-29: owner and presentation must clear in the same event turn at an\n   account boundary, including when the progress panel was open. */\nconst boundaryHandle = lb.start({ key: 'session:boundary:probe', kind: 'probe', label: 'Session boundary probe', timeoutMs: 5000 });\nassert(boundaryHandle && boundaryHandle.isCurrent(), 'session-boundary presentation probe did not start');\nps.panel.open();\nassert(nodes.mlsPsChip.classList.contains('on'), 'active boundary probe did not show progress');\nassert(nodes.mlsPsPanel.classList.contains('on'), 'boundary probe panel did not open');\ncontext.dispatchEvent(new context.CustomEvent('mls:session-boundary', { detail: { epoch: 9 } }));\nassert.strictEqual(lb.snapshot().length, 0, 'session boundary retained progress owner state');\nassert(!nodes.mlsPsChip.classList.contains('on'), 'session boundary left the prior account progress chip visible');\nassert(!nodes.mlsPsPanel.classList.contains('on'), 'session boundary left the prior account progress panel visible');\n\nconsole.log('PASS progress stages: connect/reconnect, schedule counts + stale rejection + partial honesty, history N-of-M without fake percents (incl. bodies lane), whole-pull stamp watcher with cross-tab disclosure, staging probe->confirm->write->verify, review counts, teach, session-boundary presentation reset, deadlines everywhere, loader wiring');"
  ]
]);
...
const sameTabPlan = prepare('tests/same-tab-owner-upgrade-runtime.test.js', 'utf8', [[
  ...
  "  { asset: 'feat_mls_loading_calm.js', version: 'lb-2.1.2', token: '20260729lb212a1', globalName: '__mlsLoadingCalm', speech: false },"
]]);
...
const siteAuditPlan = prepare('tests/site-audit-regressions.test.js', 'utf8', [
  ...
  [
    ...
    "loadingLoader.includes(\"s.src=A+'?v=20260729lb212a1'\")"
  ]
]);
...
const templateTestPlan = prepare('tests/template-library-runtime.test.js', 'utf8', [
  [
    ...
    "liveLoader.indexOf(\"var A='feat_mls_loading_calm.js',V='lb-2.1.2'\")"
  ],
  [
    "includes(\"s.src=A+'?v=20260719lb204'\")",
    "includes(\"s.src=A+'?v=20260729lb212a1'\")"
  ]
]);
...
console.log('Applied proposal 041: bounded progress retention with retry-cancel and session-presentation safety.');
