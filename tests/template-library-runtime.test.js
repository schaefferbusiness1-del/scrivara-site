...
function staticContracts() {
  ...
  assert(source.includes("VERSION='tl-1.3.2'"), 'the save-saves + destructive-activate-guard lane must carry tl-1.3.0');
  ...
  for (const loader of [liveLoader, stagingLoader]) {
    assert(loader.includes('feat_mls_template_library.js?v=20260731tl132'));
  }
  const loadingAt = liveLoader.indexOf("var A='feat_mls_loading_calm.js',V='lb-2.1.0'");
  const templateAt = liveLoader.indexOf('feat_mls_template_library.js?v=20260731tl132');
