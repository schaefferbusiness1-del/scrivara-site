...
for (const [name, bundle] of [['production', production], ['staging', staging]]) {
  assert(bundle.includes("V='3.2.4',api=window.__mlsRedesign") && bundle.includes("old.removeAttribute('data-mls-asset')"),
 ...
 * removed before 3.2.4 evaluates the idempotent shell builders. */
{
  const removed = [];
 ...
 * throwaway state; a navigation must still add exactly one canonical entry. */
{
  ...
  assert.strictEqual(current.__rdHistVersion, '3.2.4');
...
console.log('PASS redesign hot upgrade: 3.2.1 nodes/wrappers retire and 3.2.4 records one reversible history owner');
