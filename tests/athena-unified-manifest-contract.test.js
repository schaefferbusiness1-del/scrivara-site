...
for (const row of [billingRow, signRow]) {
  assert.strictEqual(row.capability, 'manual', `${row.id} must remain manual under current policy`);
  assert.strictEqual(row.action, '', `${row.id} must not expose an executable action`);
  assert(/directly in Athena|yourself in Athena/i.test(row.reason + ' ' + row.consequence), `${row.id} must name the manual Athena destination`);
  assert(!/update MLS Assist/i.test(row.reason + ' ' + row.consequence), `${row.id} must not advertise an upgrade that cannot change product policy`);
}
...
assert.deepStrictEqual(Object.keys(capableByAction).sort(), ['save_draft', 'write_note'], 'legacy capabilities widened the executable action set');
const capableBilling = capable.rows.find(row => row.id === 'stage-billing');
const capableSign = capable.rows.find(row => row.id === 'sign-encounter');
assert.strictEqual(capableBilling.capability, 'manual');
assert.deepStrictEqual(Array.from(capableBilling.payload.billing.cptCodes), ['20610']);
assert.strictEqual(capableSign.capability, 'manual');
assert.strictEqual(capableSign.payload.noteText, 'Full generated note.\nAssessment and plan remain intact.');
...
console.log('PASS unified Athena manifest: immutable payloads, note/save-only execution despite legacy capabilities, exact final-action payloads remain manual, and unknowns stay blocked');
