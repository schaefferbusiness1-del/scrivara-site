...
const seenSrc = between(app, '/* ===== apptdone-1.0.0 — arrival is not completion', '/* ===== end apptdone-1.0.0');
...
/* F2 may retain its historical install marker, but it must delegate to the one
   exact base predicate rather than rebuilding a second, weaker name cache. */
const f2 = between(connect, '  function installF2() {', '    /* _nextClinicDay');
assert(f2.includes('orig._seenToday(target, appointmentId)'),
  'F2 _seenToday no longer delegates to the exact base completion predicate');
assert(!f2.includes('seenTodayIndex') && !f2.includes('anyName') && !f2.includes('updated ||'),
  'F2 restored the ambiguous-name or edit-time completion cache');

console.log('PASS op-note draft quarantine: PDF/routing/sign gates fail closed on unresolved placeholders, and completion excludes drafts with exact chart/appointment evidence');
