...
function loaderOwnershipMatrix() {
  ...
  function harness(preview, fixedEntropy) {
    ...
    function openStudy(){(listeners['mls:study-lifecycle']||[]).slice().forEach(fn=>fn({detail:{reason:'render'}}));}
    return {window,document,children,sandbox,evalLoader(admit=true){vm.runInContext(loaderSource,sandbox);if(admit)openStudy();},openStudy,evalAsset(n){document.currentScript=n;vm.runInContext(source,sandbox);document.currentScript=null;}};
  }
  ...
  h=harness(true);h.evalLoader(false);eq(h.children.length,0,'occurrence joined cold boot before Study first use');h.openStudy();let ctl=h.window.__mlsP1AthenaOccurrenceLoader,node=h.children[0];ok(ctl&&node,'Study first use did not create its owned occurrence script');eq(node.getAttribute('data-mls-asset'),'feat_mls_athena_occurrence.js','loader used the wrong canonical asset identity');eq(node.getAttribute('data-mls-install-token'),ctl.installToken,'loader did not bind the script to its install token');
...
async function canonicalMutationMatrix() {
  ...
  const throwCtx={Promise,Date,Math,Object,String,Number,Array,RegExp,setInterval(fn){const id={fn};heartbeats.add(id);return id;},clearInterval(id){heartbeats.delete(id);},studyCapture(){return {generation:1};},studyCurrent(){return true;},getPatients(){return[];},normDob:ci.normDob,tagPatientCohort(){},buildApptForRow(){return{};},_assistReadChart(){reads++;return Promise.resolve({});},_athenaChartTextForParse(){},_parsePatientChart(){},_athenaChartProfileCoverage(){},_athenaNewPatientVerifiedRef(){},_savePatientChart(){},__mlsP1AthenaReadLease:{claim(){return active;},owns(){return false;},ready(){throw new Error('candidate-ready-throw');},touch(){},release(t){if(t===active){active='';released++;return true;}return false;}}};throwCtx.window=throwCtx;vm.runInNewContext(section+'\nwindow.__testImporter=verifiedCandidateImport;',throwCtx,{filename:'p1-candidate-ready.js'});
  const noProof=await throwCtx.__testImporter({name:'New Person',dob:'01/02/1980',patientId:'ATH200'},'',{});eq(noProof.code,'failed','missing canonical isCurrent proof did not fail closed');eq(released,0,'missing isCurrent proof acquired a lease');
  const throwResult=await throwCtx.__testImporter({name:'New Person',dob:'01/02/1980',patientId:'ATH200'},'',{isCurrent(){return true;}});eq(throwResult.code,'failed','candidate sync-ready throw did not settle failed');eq(released,1,'candidate sync-ready throw stranded its token');eq(active,'','candidate sync-ready throw left token active');eq(heartbeats.size,0,'candidate sync-ready throw leaked heartbeat');
}
...
async function sharedStudyReadyThrowMatrix() {
  ...
  for(const item of [{name:'report',source:report,fn:'assistReadReport',args:[()=>{}]},{name:'search',source:search,fn:'assistSearchProcedure',args:[{}, {}, ()=>{}]}]){
    ...
    const c={Promise,Date,Math,Object,String,Number,Array,RegExp,setTimeout(){return 1;},clearTimeout(){},setInterval(fn){const id={fn};intervals.add(id);return id;},clearInterval(id){intervals.delete(id);},safe(fn,fallback){try{return fn();}catch(e){return fallback;}},studyCapture(){return {generation:1};},studyCurrent(){return true;},studyCancelledError(){const e=new Error('cancelled');e.code='P1_STUDY_STALE';return e;},__mlsP1AthenaReadLease:mgr,location:{origin:'https://mlsscribe.com'},addEventListener(t,f){if(t==='message')handlers.push(f);},removeEventListener(t,f){if(t==='message'){const i=handlers.indexOf(f);if(i>=0)handlers.splice(i,1);}},postMessage(){}};c.window=c;vm.runInNewContext(item.source,c,{filename:'p1-study-'+item.name+'-ready.js'});
