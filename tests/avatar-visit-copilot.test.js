'use strict';
/*
 * AVATAR — THE VISIT COPILOT (av-5.7.4)
 * -----------------------------------------------------------------------------
...
assert(source.includes("var VERSION = 'av-5.7.4'"), 'VERSION must move with this train');
{
  const connect = fs.readFileSync(path.join(root, 'mls-connect.js'), 'utf8');
  const tokenM = connect.match(/feat_mls_avatar\.js\?v=\d{8}av(\d+)/);
  assert(tokenM && tokenM[1] === '574', 'the loader cache token must name av-5.7.4 (found ' + (tokenM && tokenM[1]) + ')');
}

console.log('PASS avatar visit copilot (av-5.7.4): detector executed on ' + (12 + REFUSALS.length + 4) +
