...
ok(ct.includes("['legFind', 'legSched', 'legOrder'].forEach(function (key) { var v = String(openedDiag[key] == null ? '' : openedDiag[key]).toLowerCase().replace(/[^a-z0-9-]/g, '').slice(0, 40); if (v) safeDiag[key] = v; });"), 'content.js copies the three keys through a closed sanitizer');
