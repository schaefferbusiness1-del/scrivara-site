...
ok(bg.includes("findByDobShape: String((frd && frd.diag && frd.diag.findByDobShape) || ''), dobFindRows: Number(frd && frd.diag && frd.diag.findRows) || 0, dobFindDobHit: Number(frd && frd.diag && frd.diag.findDobHit) || 0, dobFindNameHit: Number(frd && frd.diag && frd.diag.findNameHit) || 0, dobFindAltRows: Number(frd && frd.diag && frd.diag.findAltRows) || 0, dobFindUsedRows: Number(frd && frd.diag && frd.diag.findUsedRows) || 0, dobFindResultsTotal: Number(frd && frd.diag && frd.diag.findResultsTotal) || 0 }"), 'the ladder carries the by-DOB counts and codes (2.1.0: used rows and the results total too)');
ok(ct.includes("'dobFindUsedRows', 'dobFindResultsTotal' /* findbydob-2.1.0 (3.0.158) */"), 'content.js allowlists them');
ok(bg.includes("if (c.charAt(0) === 'l') { var __mid = 'n'; if (a.length === b.length) { __mid = 'e';"), 'same-surname shape codes carry the middle-token relation');
...
eq(shape('Bill Souza', 'William Souza'), 'lxe22', 'same last, different first (no middle tokens: e)');
eq(shape('Will Souza', 'William Souza'), 'lpe22', 'same last, first a prefix');
eq(shape('Bill M de Souza', 'William Michael de Souza'), 'lxp44', 'same last, first differs, a middle initial against the full middle name: p');
eq(shape('Bill M de Souza', 'William J de Souza'), 'lxx44', 'a different middle: x');
eq(shape('Bill Souza', 'William M Souza'), 'lxn23', 'token counts differ: n');
...
const usStart = bg.indexOf('function __usedShape(req, row) {'); const usEndTok = '/* findbydob-2.1.0 (3.0.158): a middle initial against the full middle name */ return false; } return true; }'; const usEnd = bg.indexOf(usEndTok, usStart);
...
eq(usedShape('Bill M de Souza', 'William Michael de Souza'), true, 'findbydob-2.1.0: a middle initial against the full middle name');
eq(usedShape('Bill Michael de Souza', 'William M de Souza'), true, 'and the reverse');
eq(usedShape('Bill M de Souza', 'William M da Souza'), false, 'a different particle is not (particles are not prefixes of each other)');
eq(usedShape('Bill M de Souza', 'William M de Souz'), false, 'the last token must be exact even when one is a prefix of the other');
eq(usedShape('Bill M Souza', 'William Ma Souza'), true, 'a two-letter middle that starts with the initial still counts as the same middle');
...
const e = bg.indexOf("dobFindResultsTotal: Number(frd && frd.diag && frd.diag.findResultsTotal) || 0 }); } catch (eFrd) {} }", s); ok(e > s, 'ladder seam present');
