...
assert(source.includes("var VERSION = 'av-1.0.0'"), 'version token moved without updating this contract');
...
const marker = "feat_mls_avatar.js?v=20260805av100";
assert(connect.indexOf(marker) >= 0, 'mls-connect.js is missing the av100 loader');
...
(async function main() {
  // fail-closed chart resolution
  {
    ...
    assert.strictEqual(window.__mlsAvatar.version, 'av-1.0.0');
  ...
  // idempotent import with provenance stamp
  {
    const patient = { id: 'ext-9', name: 'Exact Patient', summary: 'Existing history.' };
    const { window } = build([patient]);
    const saved = [];
    window.upsertPatient = (p) => saved.push(JSON.parse(JSON.stringify(p)));
    const checkin = { patient_external_id: 'ext-9', ready_at: '2026-08-05 15:00:00', summary: 'Patient reports knee pain 4/10.' };
    ...
    assert(/\[Avatar check-in — completed .*\]/.test(saved[0].summary), 'the provenance stamp is present');
    assert(/knee pain 4\/10/.test(saved[0].summary));
    // the app store now holds the updated summary; a second tap must not append again
    patient.summary = saved[0].summary;
