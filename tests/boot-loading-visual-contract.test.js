...
assert(app.includes('const SF_GATE_MIN_MS=1800, SF_GATE_MAX_MS=32000, SF_GATE_QUIET_MS=700'), 'bounded, quiet-window loading contract was lost');
...
assert(app.includes("window.__MLS_AV='b410'"), 'ScribeFlow loader was not cache-busted to b410');
...
assert(connect.includes("window.__MLS_AV = window.__MLS_AV || 'b410'"), 'shared asset version was not bumped to b410');
assert(connect.includes("var MLS_APP_BUILD='2026-07-18-b410'"), 'app build was not bumped to b410');
