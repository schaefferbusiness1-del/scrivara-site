...
assert(read('ScribeFlow.html').includes("window.__MLS_AV='b1023'"), 'web asset stamp changed from b1023');
assert(read('sw.js').includes("const CACHE = 'mls-v208'"), 'service-worker cache stamp changed from v64');console.log('PASS same-tab UI account isolation: A -> logout -> B clears PHI/modals/intake/prompts, restores role markup, and reconciles account-gated menu rows');
