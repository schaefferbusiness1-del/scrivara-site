...
assert(read('ScribeFlow.html').includes("window.__MLS_AV='b968'"), 'web asset stamp changed from b968');
assert(read('sw.js').includes("const CACHE = 'mls-v201'"), 'service-worker cache stamp changed from v64');console.log('PASS same-tab UI account isolation: A -> logout -> B clears PHI/modals/intake/prompts, restores role markup, and reconciles account-gated menu rows');
