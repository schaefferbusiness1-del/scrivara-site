...
assert(read('ScribeFlow.html').includes("window.__MLS_AV='b941'"), 'web asset stamp changed from b941');
assert(read('sw.js').includes("const CACHE = 'mls-v198'"), 'service-worker cache stamp changed from v64');console.log('PASS same-tab UI account isolation: A -> logout -> B clears PHI/modals/intake/prompts, restores role markup, and reconciles account-gated menu rows');
