...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.76.zip';
...
const RELEASED_PACKAGE_MIRROR = 'MLS_Assist_v3.0.76.bin';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.76.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.76.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, '445d57e0ee0962a75ab79033e48268e379c6d8dbe2afc90894727a128479489c',
...
const mirrorSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.76.bin'))).digest('hex');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3.0.76\.zip["']/i.test(extensionPage) &&
  /445d57e0ee0962a75ab79033e48268e379c6d8dbe2afc90894727a128479489c/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.76.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.76.zip`), 'the released package download must reach the network');
