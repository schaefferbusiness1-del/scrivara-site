...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.47.zip';
...
const RELEASED_PACKAGE_MIRROR = 'MLS_Assist_v3.0.47.bin';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.47.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.47.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, '81c7bd84535047757734bdd2879e1132401a13e403cd4d74d74c59eed87ae113',
...
const mirrorSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.47.bin'))).digest('hex');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3.0.47\.zip["']/i.test(extensionPage) &&
  /81c7bd84535047757734bdd2879e1132401a13e403cd4d74d74c59eed87ae113/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.47.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.47.zip`), 'the released package download must reach the network');
