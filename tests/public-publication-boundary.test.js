...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.29.zip';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.29.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.29.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, 'c124d56497bd5c82d345022b2fc47d315181d23ef6e02cee064e964cc1776d4e',
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3\.0\.29\.zip["']/i.test(extensionPage) &&
  /c124d56497bd5c82d345022b2fc47d315181d23ef6e02cee064e964cc1776d4e/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.29.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.29.zip`), 'the released package download must reach the network');
