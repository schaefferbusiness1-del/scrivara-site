...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.50.zip';
...
const RELEASED_PACKAGE_MIRROR = 'MLS_Assist_v3.0.50.bin';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.50.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.50.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, '5e107b2c454e7320bb7344a0388ffe0871fa69342f3bfe0bd4a93f0b471732f3',
...
const mirrorSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.50.bin'))).digest('hex');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3.0.50\.zip["']/i.test(extensionPage) &&
  /5e107b2c454e7320bb7344a0388ffe0871fa69342f3bfe0bd4a93f0b471732f3/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.50.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.50.zip`), 'the released package download must reach the network');
