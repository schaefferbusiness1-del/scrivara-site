...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.25.zip';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.25.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.25.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, 'd007a87f612152a41dd30d8099ad7a38a850bc2a120bcae059175346940600ed',
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3\.0\.25\.zip["']/i.test(extensionPage) &&
  /d007a87f612152a41dd30d8099ad7a38a850bc2a120bcae059175346940600ed/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.25.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.25.zip`), 'the released package download must reach the network');
