...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.72.zip';
...
const RELEASED_PACKAGE_MIRROR = 'MLS_Assist_v3.0.72.bin';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.72.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.72.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, 'a1dca473780e1dd2b20075c3d3869a3c3ffcf96c1d22cd53e5a91eb0535efe74',
...
const mirrorSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.72.bin'))).digest('hex');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3.0.72\.zip["']/i.test(extensionPage) &&
  /a1dca473780e1dd2b20075c3d3869a3c3ffcf96c1d22cd53e5a91eb0535efe74/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.72.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.72.zip`), 'the released package download must reach the network');
