...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.30.zip';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.30.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.30.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, 'dc03b20dde890cd8292b8d89c66fb1e317df77167a40d3f82e5375516f4a9363',
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3\.0\.30\.zip["']/i.test(extensionPage) &&
  /dc03b20dde890cd8292b8d89c66fb1e317df77167a40d3f82e5375516f4a9363/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.30.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.30.zip`), 'the released package download must reach the network');
