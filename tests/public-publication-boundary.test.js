...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.51.zip';
...
const RELEASED_PACKAGE_MIRROR = 'MLS_Assist_v3.0.51.bin';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.51.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.51.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, '6523fa1cf5cb30fcc2f45c214626e137c9516f296c56836219be7a5b453329f8',
...
const mirrorSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.51.bin'))).digest('hex');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3.0.51\.zip["']/i.test(extensionPage) &&
  /6523fa1cf5cb30fcc2f45c214626e137c9516f296c56836219be7a5b453329f8/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.51.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.51.zip`), 'the released package download must reach the network');
