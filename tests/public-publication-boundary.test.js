...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.48.zip';
...
const RELEASED_PACKAGE_MIRROR = 'MLS_Assist_v3.0.48.bin';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.48.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.48.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, 'b8bc12fb4544365bea20ac7971207c095355c92de290b8322a5cc12d84f826a4',
...
const mirrorSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.48.bin'))).digest('hex');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3.0.48\.zip["']/i.test(extensionPage) &&
  /b8bc12fb4544365bea20ac7971207c095355c92de290b8322a5cc12d84f826a4/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.48.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.48.zip`), 'the released package download must reach the network');
  ...
  await activateWork;
  assert.deepStrictEqual(await cacheApi.keys(), ['mls-v200'], 'activation must remove every superseded MLS cache');
