...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.23.zip';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.23.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.23.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, '869574331f9ba95c2a9330d807ae907108a3020c4ecc492116f732a7739873e7',
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3\.0\.23\.zip["']/i.test(extensionPage) &&
  /869574331f9ba95c2a9330d807ae907108a3020c4ecc492116f732a7739873e7/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.23.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.23.zip`), 'the released package download must reach the network');
