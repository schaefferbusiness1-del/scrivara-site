...
/* Owner directive 2026-08-26: the exact stamped 3.0.101 release ships publicly;
 * its bytes are digest-pinned below. Candidates stay excluded. */
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.101.zip';
const RELEASED_PACKAGE_SHA256 = '1ac951996f487f9227ac58411c907ca1416d452eaecb078dd13afb4a39865fa2';
assert(/^[a-f0-9]{64}$/.test(RELEASED_PACKAGE_SHA256),
  '3.0.101 package digest must be stamped after deterministic packaging before publication');
...
const RELEASED_PACKAGE_MIRROR = 'MLS_Assist_v3.0.101.bin';
...
/* Owner directive 2026-08-26: EXACTLY the stamped 3.0.101 release is public.
 * Its published bytes must equal the release digest — any drift fails. */
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.101.zip'],
  'exactly the released 3.0.101 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.101.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, RELEASED_PACKAGE_SHA256,
  'published package bytes must be the exact stamped 3.0.101 release');
...
const mirrorSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.101.bin'))).digest('hex');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3.0.101\.zip["']/i.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.101.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.101.zip`), 'the released package download must reach the network');
