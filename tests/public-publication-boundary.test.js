...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.74.zip';
...
const RELEASED_PACKAGE_MIRROR = 'MLS_Assist_v3.0.74.bin';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.74.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.74.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, '70c7a57ca80bf58a946531775d6ab0398b9fd82558bb435a20c62d0b4909ac72',
...
const mirrorSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.74.bin'))).digest('hex');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3.0.74\.zip["']/i.test(extensionPage) &&
  /70c7a57ca80bf58a946531775d6ab0398b9fd82558bb435a20c62d0b4909ac72/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.74.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.74.zip`), 'the released package download must reach the network');
