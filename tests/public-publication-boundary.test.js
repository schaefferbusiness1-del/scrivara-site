...
/* Owner directive 2026-07-20: the exact stamped 3.0.4 release ships publicly;
 * its bytes are digest-pinned below. Candidates stay excluded. */
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.4.zip';
...
/* Owner directive 2026-07-20: EXACTLY the stamped 3.0.4 release is public.
 * Its published bytes must equal the release digest — any drift fails. */
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.4.zip'],
  'exactly the released 3.0.4 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.4.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, '953f5bebf13a0aaa24cbc56db27664a242aef60a1f96dc84beacb20e11334e8a',
  'published package bytes must be the exact stamped 3.0.4 release');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3\.0\.4\.zip["']/i.test(extensionPage) &&
  /953f5bebf13a0aaa24cbc56db27664a242aef60a1f96dc84beacb20e11334e8a/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  /* The exact released 3.0.4 package passes through to the network (never 410,
   * never cached — the cached-keys assertion below covers every ZIP). */
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.4.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.4.zip`), 'the released package download must reach the network');
  ...
  await activateWork;
  assert.deepStrictEqual(await cacheApi.keys(), ['mls-v112'], 'activation must remove every superseded MLS cache');
