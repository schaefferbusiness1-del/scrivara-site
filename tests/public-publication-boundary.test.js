...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.52.zip';
...
const RELEASED_PACKAGE_MIRROR = 'MLS_Assist_v3.0.52.bin';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.52.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.52.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, '22c415c4829df79ce3fde8990992bb903431237c3a86a95a249578287738b99e',
...
const mirrorSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.52.bin'))).digest('hex');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3.0.52\.zip["']/i.test(extensionPage) &&
  /22c415c4829df79ce3fde8990992bb903431237c3a86a95a249578287738b99e/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.52.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.52.zip`), 'the released package download must reach the network');
  ...
  await activateWork;
  assert.deepStrictEqual(await cacheApi.keys(), ['mls-v204'], 'activation must remove every superseded MLS cache');
