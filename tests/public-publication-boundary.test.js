...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.42.zip';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.42.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.42.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, '9b5ef9784a7943379efe4bcd915e245edf39cf672606eb871d117ae64ddffcdc',
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3\.0\.42\.zip["']/i.test(extensionPage) &&
  /9b5ef9784a7943379efe4bcd915e245edf39cf672606eb871d117ae64ddffcdc/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.42.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.42.zip`), 'the released package download must reach the network');
  ...
  const newFindAsset = await runFetch(`${origin}/feat_mls_fixpack_0701.js?v=20260802fp116`);
  assert.strictEqual(await newFindAsset.text(), `network:${origin}/feat_mls_fixpack_0701.js?v=20260802fp116`,
    'old fp110 cache entry shadowed the exact-ID/account-bound fp111 Find implementation');
  assert(fetchCalls.includes(`${origin}/feat_mls_fixpack_0701.js?v=20260802fp116`),
  ...
  assert(cachedKeys.includes(`${origin}/feat_mls_fixpack_0701.js?v=20260802fp116`), 'new exact-ID Find implementation was not cached under its own immutable URL');
  ...
  await activateWork;
  assert.deepStrictEqual(await cacheApi.keys(), ['mls-v196'], 'activation must remove every superseded MLS cache');
