...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.49.zip';
...
const RELEASED_PACKAGE_MIRROR = 'MLS_Assist_v3.0.49.bin';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.49.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.49.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, '05027f4a17e15cbcfb34f8f08c96f2cd4e6c52cd4325a6e633421b2cba8fa217',
...
const mirrorSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.49.bin'))).digest('hex');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3.0.49\.zip["']/i.test(extensionPage) &&
  /05027f4a17e15cbcfb34f8f08c96f2cd4e6c52cd4325a6e633421b2cba8fa217/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.49.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.49.zip`), 'the released package download must reach the network');
  ...
  await activateWork;
  assert.deepStrictEqual(await cacheApi.keys(), ['mls-v201'], 'activation must remove every superseded MLS cache');
