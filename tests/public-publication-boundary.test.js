...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.26.zip';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.26.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.26.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, '128d60fae14dc3cf49627c8a29a6729a3fe62910a264a15cc0c5f35334298f24',
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3\.0\.26\.zip["']/i.test(extensionPage) &&
  /128d60fae14dc3cf49627c8a29a6729a3fe62910a264a15cc0c5f35334298f24/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.26.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.26.zip`), 'the released package download must reach the network');
