...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.53.zip';
...
const RELEASED_PACKAGE_MIRROR = 'MLS_Assist_v3.0.53.bin';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.53.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.53.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, 'cd89ee7ed85528061ef7019821d644b6dcec74e433b1fd2249afad435aa09414',
...
const mirrorSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.53.bin'))).digest('hex');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3.0.53\.zip["']/i.test(extensionPage) &&
  /cd89ee7ed85528061ef7019821d644b6dcec74e433b1fd2249afad435aa09414/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.53.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.53.zip`), 'the released package download must reach the network');
