...
/* Owner directive 2026-07-20: the exact stamped 3.0.1 release ships publicly;
 * its bytes are digest-pinned below. Candidates stay excluded. */
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.1.zip';
...
/* Owner directive 2026-07-20: EXACTLY the stamped 3.0.1 release is public.
 * Its published bytes must equal the release digest — any drift fails. */
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.1.zip'],
  'exactly the released 3.0.1 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.1.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, '5c0d678a1a8e265122e93340063d3010c4a5f2c200c4f21d681f8bf9b47178aa',
  'published package bytes must be the exact stamped 3.0.1 release');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3\.0\.1\.zip["']/i.test(extensionPage) &&
  /5c0d678a1a8e265122e93340063d3010c4a5f2c200c4f21d681f8bf9b47178aa/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  /* The exact released 3.0.1 package passes through to the network (never 410,
   * never cached — the cached-keys assertion below covers every ZIP). */
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.1.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.1.zip`), 'the released package download must reach the network');
  ...
  const newFindAsset = await runFetch(`${origin}/feat_mls_fixpack_0701.js?v=20260722fp112`);
  assert.strictEqual(await newFindAsset.text(), `network:${origin}/feat_mls_fixpack_0701.js?v=20260722fp112`,
    'old fp110 cache entry shadowed the exact-ID/account-bound fp111 Find implementation');
  assert(fetchCalls.includes(`${origin}/feat_mls_fixpack_0701.js?v=20260722fp112`),
  ...
  assert(cachedKeys.includes(`${origin}/feat_mls_fixpack_0701.js?v=20260722fp112`), 'new exact-ID Find implementation was not cached under its own immutable URL');
  ...
  await activateWork;
  assert.deepStrictEqual(await cacheApi.keys(), ['mls-v76'], 'activation must remove every superseded MLS cache');
