...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.31.zip';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.31.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.31.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, 'dedfeacdfda3962b0ae00eb28abca07485ea09a3ce9eedf65e299c03ee9c68e8',
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3\.0\.31\.zip["']/i.test(extensionPage) &&
  /dedfeacdfda3962b0ae00eb28abca07485ea09a3ce9eedf65e299c03ee9c68e8/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.31.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.31.zip`), 'the released package download must reach the network');
