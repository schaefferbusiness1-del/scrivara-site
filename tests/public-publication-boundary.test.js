...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.44.zip';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.44.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.44.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, '54622758c4e1543c24560a9bba649ad446ebebaabf47052aab1174fcd6f4bb77',
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3\.0\.44\.zip["']/i.test(extensionPage) &&
  /54622758c4e1543c24560a9bba649ad446ebebaabf47052aab1174fcd6f4bb77/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.44.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.44.zip`), 'the released package download must reach the network');
