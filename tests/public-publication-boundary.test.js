...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.57.zip';
...
const RELEASED_PACKAGE_MIRROR = 'MLS_Assist_v3.0.57.bin';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.57.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.57.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, '0efd00ccf06a48d8e8a5992681a2e58d52105fcc3ff661c884f2d4d9303575df',
...
const mirrorSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.57.bin'))).digest('hex');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3.0.57\.zip["']/i.test(extensionPage) &&
  /0efd00ccf06a48d8e8a5992681a2e58d52105fcc3ff661c884f2d4d9303575df/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.57.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.57.zip`), 'the released package download must reach the network');
