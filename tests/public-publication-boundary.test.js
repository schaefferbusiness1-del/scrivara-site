...
/* Owner directive 2026-07-20: the exact stamped 3.0.2 release ships publicly;
 * its bytes are digest-pinned below. Candidates stay excluded. */
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.2.zip';
...
/* Owner directive 2026-07-20: EXACTLY the stamped 3.0.2 release is public.
 * Its published bytes must equal the release digest — any drift fails. */
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.2.zip'],
  'exactly the released 3.0.2 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.2.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, 'b3012cb8da74788b29be9f52d7fdb28d8a2871f9a002f8cfd84b41d29cb2dbc2',
  'published package bytes must be the exact stamped 3.0.2 release');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3\.0\.2\.zip["']/i.test(extensionPage) &&
  /b3012cb8da74788b29be9f52d7fdb28d8a2871f9a002f8cfd84b41d29cb2dbc2/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  /* The exact released 3.0.2 package passes through to the network (never 410,
   * never cached — the cached-keys assertion below covers every ZIP). */
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.2.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.2.zip`), 'the released package download must reach the network');
  ...
  await activateWork;
  assert.deepStrictEqual(await cacheApi.keys(), ['mls-v80'], 'activation must remove every superseded MLS cache');
