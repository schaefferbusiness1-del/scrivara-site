...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.54.zip';
...
const RELEASED_PACKAGE_MIRROR = 'MLS_Assist_v3.0.54.bin';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.54.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.54.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, '34041bcfc076ef3da87a8ab6d8300a720ff6fa5dca405d84be328894241bb5c8',
...
const mirrorSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.54.bin'))).digest('hex');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3.0.54\.zip["']/i.test(extensionPage) &&
  /34041bcfc076ef3da87a8ab6d8300a720ff6fa5dca405d84be328894241bb5c8/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.54.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.54.zip`), 'the released package download must reach the network');
  ...
  await activateWork;
  assert.deepStrictEqual(await cacheApi.keys(), ['mls-v205'], 'activation must remove every superseded MLS cache');
