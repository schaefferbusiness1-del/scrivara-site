...
/* Owner directive 2026-08-26: the exact stamped 3.0.112 release ships publicly;
 * its bytes are digest-pinned below. Candidates stay excluded. */
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.112.zip';
const RELEASED_PACKAGE_SHA256 = 'f7f5ece88cb0d49125595060d6560bb138bd9ee0426972ea17fe1c696a6ac929';
assert(/^[a-f0-9]{64}$/.test(RELEASED_PACKAGE_SHA256),
  '3.0.112 package digest must be stamped after deterministic packaging before publication');
...
const RELEASED_PACKAGE_MIRROR = 'MLS_Assist_v3.0.112.bin';
...
/* Owner directive 2026-08-26: EXACTLY the stamped 3.0.112 release is public.
 * Its published bytes must equal the release digest — any drift fails. */
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.112.zip'],
  'exactly the released 3.0.112 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.112.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, RELEASED_PACKAGE_SHA256,
  'published package bytes must be the exact stamped 3.0.112 release');
...
const mirrorSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.112.bin'))).digest('hex');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3.0.112\.zip["']/i.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.112.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.112.zip`), 'the released package download must reach the network');
  ...
  await activateWork;
   assert.deepStrictEqual(await cacheApi.keys(), ['mls-v239'], 'activation must remove every superseded MLS cache');
