...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.77.zip';
...
const RELEASED_PACKAGE_MIRROR = 'MLS_Assist_v3.0.77.bin';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.77.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.77.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, '3de8327eeb2c4b0d84c464d3b47252811c2e167895a195dc64418bf4db7bf4cd',
...
const mirrorSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.77.bin'))).digest('hex');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3.0.77\.zip["']/i.test(extensionPage) &&
  /3de8327eeb2c4b0d84c464d3b47252811c2e167895a195dc64418bf4db7bf4cd/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.77.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.77.zip`), 'the released package download must reach the network');
