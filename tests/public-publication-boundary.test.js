...
async function verifyServiceWorkerRuntime() {
  ...
  const newFindAsset = await runFetch(`${origin}/feat_mls_fixpack_0701.js?v=20260722fp113`);
  assert.strictEqual(await newFindAsset.text(), `network:${origin}/feat_mls_fixpack_0701.js?v=20260722fp113`,
    'old fp110 cache entry shadowed the exact-ID/account-bound fp111 Find implementation');
  assert(fetchCalls.includes(`${origin}/feat_mls_fixpack_0701.js?v=20260722fp113`),
  ...
  assert(cachedKeys.includes(`${origin}/feat_mls_fixpack_0701.js?v=20260722fp113`), 'new exact-ID Find implementation was not cached under its own immutable URL');
  ...
  await activateWork;
  assert.deepStrictEqual(await cacheApi.keys(), ['mls-v85'], 'activation must remove every superseded MLS cache');
