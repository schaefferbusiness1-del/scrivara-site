...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.55.zip';
...
const RELEASED_PACKAGE_MIRROR = 'MLS_Assist_v3.0.55.bin';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.55.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.55.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, 'dd5738e227cfe27701edf74703d404b097857a667fdb199c4f3b980616dfc5c1',
...
const mirrorSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.55.bin'))).digest('hex');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3.0.55\.zip["']/i.test(extensionPage) &&
  /dd5738e227cfe27701edf74703d404b097857a667fdb199c4f3b980616dfc5c1/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.55.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.55.zip`), 'the released package download must reach the network');
