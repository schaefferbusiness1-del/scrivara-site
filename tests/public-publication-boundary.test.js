...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.56.zip';
...
const RELEASED_PACKAGE_MIRROR = 'MLS_Assist_v3.0.56.bin';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.56.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.56.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, 'a89dcfd772ef80c159be0e14d34a66a2905abcf9827a28a4f94a4da201e82f00',
...
const mirrorSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.56.bin'))).digest('hex');
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3.0.56\.zip["']/i.test(extensionPage) &&
  /a89dcfd772ef80c159be0e14d34a66a2905abcf9827a28a4f94a4da201e82f00/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.56.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.56.zip`), 'the released package download must reach the network');
