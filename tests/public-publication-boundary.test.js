...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.28.zip';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.28.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.28.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, 'da548e7dfdc63c24c7a1ce10cc192ee1c02e5e76af7cc2f5c314e54552564a12',
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3\.0\.28\.zip["']/i.test(extensionPage) &&
  /da548e7dfdc63c24c7a1ce10cc192ee1c02e5e76af7cc2f5c314e54552564a12/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.28.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.28.zip`), 'the released package download must reach the network');
