...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.24.zip';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.24.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.24.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, '76dc252fd21694e30617aa39e76e72863499462046186907afe7c45ffacd894a',
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3\.0\.24\.zip["']/i.test(extensionPage) &&
  /76dc252fd21694e30617aa39e76e72863499462046186907afe7c45ffacd894a/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.24.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.24.zip`), 'the released package download must reach the network');
