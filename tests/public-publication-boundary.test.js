...
const RELEASED_PACKAGE = 'MLS_Assist_v3.0.27.zip';
...
assert.deepStrictEqual(zipFiles.filter((name) => includeSet.has(name)), ['MLS_Assist_v3.0.27.zip'],
  'exactly the released 3.0.22 package may be published — nothing else, and never a candidate');
const releasedZipSha = crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'MLS_Assist_v3.0.27.zip'))).digest('hex');
assert.strictEqual(releasedZipSha, 'af9a4cbeb401b1231e583541f69031dcb4def3088f44defb678d2eefa9a7721e',
...
assert(/id=["']dl["'][^>]*href=["']MLS_Assist_v3\.0\.27\.zip["']/i.test(extensionPage) &&
  /af9a4cbeb401b1231e583541f69031dcb4def3088f44defb678d2eefa9a7721e/.test(extensionPage) &&
...
async function verifyServiceWorkerRuntime() {
  ...
  const releasedArchive = await runFetch(`${origin}/MLS_Assist_v3.0.27.zip`);
  assert.notStrictEqual(releasedArchive.status, 410, 'the exact released package must pass through the service worker');
  assert(fetchCalls.includes(`${origin}/MLS_Assist_v3.0.27.zip`), 'the released package download must reach the network');
