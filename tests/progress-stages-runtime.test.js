...
assert(ps && ps.installed && ps.version === 'ps-1.4.0', 'progress-stages module missing');
...
const psLoader = connect.split(/\r?\n/).find(line => line.includes("var A='feat_mls_progress_stages.js',V='ps-1.4.0'")) || '';
...
assert(psLoader.includes("s.src=A+'?v=20260826ps140'") && psLoader.includes("s.setAttribute('data-mls-version',V)"), 'ps loader lost its exact version-aware cache token/tag');
