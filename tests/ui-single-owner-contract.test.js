...
assert(ui.includes("portalObserver = new MutationObserver(function (muts) { scheduleObservedConcern('portal', muts); })") && ui.includes("if (queue.portal && concernNeedsWork('portal')) safe(reconcilePortalOwner)"), 'portal mutations are not dirty-filtered and routed only to portal ownership reconciliation');
