...
(function(){
  ...
  function render(){
    ...
    host.querySelector('#emrSubmit').onclick=function(){
      ...
      try{ document.dispatchEvent(new CustomEvent('mlsEmrSubmit',{detail:payload})); }catch(e){}
      var narr=sections.filter(function(x){return x.kind==='narrative';}).length, act=sections.length-narr;
      host.querySelector('#emrHint').innerHTML='<b>Queued for athena (stage-only):</b> '+narr+' note section(s) go to the clinical note; '+act+' order/Rx/referral(s) get staged in athena for your final sign. Nothing fires on its own — the MLS extension places it and you confirm. If nothing happens in athena, your extension needs the update that adds athena placement.';
      var b=host.querySelector('#emrSubmit'); b.textContent='Submitted ✓'; setTimeout(function(){ b.innerHTML='Confirm &amp; submit all → athena'; },1800);
    };
...
;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_upnow_realtime.js"]'))return;var s=document.createElement('script');s.src='feat_mls_upnow_realtime.js?v='+(window.__MLS_AV||Date.now());s.setAttribute('data-mls-asset','feat_mls_upnow_realtime.js');s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* item39: UP NOW real-time honesty / no-more-patients-today banner (additive, reversible: window.__mlsUpNowRealtime.revert()) */
...
(function(){
  ...
  /* Compatibility API only. Canonical Find owns the only Ctrl/Cmd+K capture
     listener (feat_mls_command_palette.js). Keeping this old listener alive
     made both overlays open in one same-document event because
     stopPropagation() does not stop sibling listeners on document. */
  function canonicalOpen(){
    close();
    try{ if(typeof window.mlsQuickFind==='function'){ window.mlsQuickFind(); return true; } }catch(e){}
    return false;
  }
  ...
  window.__mlsCmdK={ open:canonicalOpen, close:canonicalClose, toggle:canonicalOpen, compatibilityOnly:true, owner:'canonical-find' };
})();
...
(function(){
  ...
  function safe(fn,d){ try{ return fn(); }catch(e){ return d; } }
  function esc(s){ return String(s==null?'':s).replace(/[&<>"']/g,function(c){return ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'})[c];}); }
  function email(){ return safe(function(){
    var e='';
    if(typeof window.getSessionEmail==='function')e=trim(window.getSessionEmail());
    if(!e&&typeof session!=='undefined'&&session)e=trim(session.email);
    if(!e&&window.bkUser)e=trim(window.bkUser.email);
    e=e.toLowerCase();
    return /^[\w.+-]+@[\w.-]+\.\w+$/.test(e)?e:null;
  }, null); }
  ...
  function timeValue(v){ if(!v)return 0; if(typeof v==='number')return v>1e12?v:v*1000; var d=new Date(v); return isNaN(d.getTime())?0:d.getTime(); }
  function activePatientSnapshot(){
    return safe(function(){
      var id=activeId(), p=id&&window.findPatient&&window.findPatient(id);
      if(!p&&typeof window.activePatient==='function')p=window.activePatient();
      var patientId=trim(p&&(p.id||p.patientId||p.patient_external_id)||id);
      if(!p||!patientId)return null;
      return {patientId:patientId,name:trim(p.name),dob:trim(p.dob),mrn:trim(p.mrn||p.athenaId||p.athenaPatientId)};
    },null);
  }
  function freezePatient(){ var p=activePatientSnapshot(); if(!p)return null; return safe(function(){return Object.freeze({patientId:p.patientId,name:p.name,dob:p.dob,mrn:p.mrn});},p); }
  function mark(ev,status){
    if(typeof ev==='string')ev={label:ev,status:status||'pending'};
    ev=ev||{}; ev.ts=ev.ts||Date.now(); if(!ev.patient) ev.patient=activeName(); if(ev.patientId==null) ev.patientId=activeId();
    var l=getLog(), requestId=trim(ev.requestId), replaced=false;
    if(requestId){ for(var i=l.length-1;i>=0;i--){ if(trim(l[i]&&l[i].requestId)===requestId){ l[i]=Object.assign({},l[i],ev); replaced=true; break; } } }
    if(!replaced)l.push(ev); setLog(l); render(); refreshOpenPop();
  }
  function verifyKey(){ var k=key(); return k?(k+'::activePatientVerify'):null; }
  function getVerifyReceipts(){ var k=verifyKey(); if(!k)return{}; return safe(function(){var v=JSON.parse(localStorage.getItem(k)||'{}');return v&&typeof v==='object'?v:{};},{}); }
  function getVerifyReceipt(patientId){ var id=trim(patientId),r=PASSIVE.verifyReceipts[id]||getVerifyReceipts()[id]; return r&&r.patientId===id?r:null; }
  function saveVerifyReceipt(r){ if(!r||!r.patientId)return;PASSIVE.verifyReceipts[r.patientId]=r;var k=verifyKey();if(!k)return; safe(function(){var all=getVerifyReceipts();all[r.patientId]=r;localStorage.setItem(k,JSON.stringify(all));}); }
  function sameIdentity(p,c){
    p=p||{};c=c||{};
    var name=trim(c.patientName||c.name),dob=trim(c.dob),mrn=trim(c.mrn);
    return !!(trim(p.patientId)&&normName(p.name)&&dateKey(p.dob)&&normName(p.name)===normName(name)&&dateKey(p.dob)===dateKey(dob)&&digits(mrn)&&(!digits(p.mrn)||digits(p.mrn)===digits(mrn)));
  }
  function dateKey(v){var s=trim(v),m=/(\d{4})-(\d{1,2})-(\d{1,2})/.exec(s);if(m)return Number(m[2])+'/'+Number(m[3])+'/'+m[1];m=/([01]?\d)[\/\-.]([0-3]?\d)[\/\-.](\d{2,4})/.exec(s);if(!m)return'';var y=m[3];if(y.length===2)y=(Number(y)>50?'19':'20')+y;return Number(m[1])+'/'+Number(m[2])+'/'+y;}
  function localVisitContext(patientId){
    return safe(function(){
      var b=(typeof currentVisitAthenaBinding!=='undefined'&&currentVisitAthenaBinding)||window.currentVisitAthenaBinding||null;
      if(!b||trim(b.patient&&(b.patient.patientId||b.patient.id))!==trim(patientId))return null;
      var c=b.visitContext||{};
      return {bindingId:trim(b.id),visitDate:dateKey(c.visitDate||b.displayDate),provider:normName(c.provider||b.displayProvider),appointmentId:trim(c.appointmentId),encounterId:digits(c.encounterId),encounterUrl:trim(c.encounterUrl)};
    },null);
  }
  function verifyFreshness(patient,receipt){
    if(!patient||!receipt||receipt.readOnly!==true||receipt.patientId!==patient.patientId)return{fresh:false,reason:'No exact-patient verification receipt'};
    if(!sameIdentity(patient,receipt.context||{}))return{fresh:false,reason:'Patient identity changed'};
    var at=timeValue(receipt.verifiedAt),age=Date.now()-at;if(!at||age>15*60*1000)return{fresh:false,reason:'Verification is stale — verify again'};
    var nowCtx=localVisitContext(patient.patientId),oldCtx=receipt.localContext||null;
    if(!!oldCtx!==!!nowCtx)return{fresh:false,reason:'Visit context changed — verify again'};
    if(oldCtx&&nowCtx){
      if(oldCtx.bindingId&&nowCtx.bindingId&&oldCtx.bindingId!==nowCtx.bindingId)return{fresh:false,reason:'Visit context changed — verify again'};
      if(oldCtx.visitDate&&nowCtx.visitDate&&oldCtx.visitDate!==nowCtx.visitDate)return{fresh:false,reason:'Visit date changed — verify again'};
      if(oldCtx.provider&&nowCtx.provider&&oldCtx.provider!==nowCtx.provider)return{fresh:false,reason:'Provider context changed — verify again'};
      if(oldCtx.appointmentId&&nowCtx.appointmentId&&oldCtx.appointmentId!==nowCtx.appointmentId)return{fresh:false,reason:'Appointment changed — verify again'};
      if(oldCtx.encounterId&&nowCtx.encounterId&&oldCtx.encounterId!==nowCtx.encounterId)return{fresh:false,reason:'Encounter changed — verify again'};
    }
    return{fresh:true,reason:'',ageMs:age};
  }
  function connMode(){
    return safe(function(){
  ...
  function sendStatus(e){
    var s=trim(e&&e.status).toLowerCase();
    if(s==='verified'||(s==='sent'&&e&&e.receiptVerified===true))return'verified';
    if(s==='pending')return Date.now()-Number(e&&e.ts||0)>3*60*1000?'uncertain':'pending';
    if(s==='failed'||s==='error'||s==='blocked')return'failed';
    return'uncertain';
  }
  function patientLog(patientId){ return getLog().filter(function(e){return trim(e&&e.patientId)===trim(patientId);}); }
  function sendCounts(patientId){
    var out={verified:0,pending:0,failed:0,uncertain:0,total:0};
    /* Only mutation attempts belong in the send badge. Read-only review
       openings, clipboard actions, and Athena pulls are not failed sends. */
    patientLog(patientId).forEach(function(e){if(!e||e.dir!=='push')return;var s=sendStatus(e);out[s]++;out.total++;});
    return out;
  }
  function patientStatus(){
    var snap=activePatientSnapshot(); if(!snap)return{patient:null,schedule:null,chart:null,history:null,verify:null,sends:sendCounts('')};
    var source=safe(function(){return window.findPatient&&window.findPatient(snap.patientId);},null)||{};
    var appts=safe(function(){return (Array.isArray(window._calAppts)?window._calAppts:[]).filter(function(a){return trim(a&&(a.patient_external_id||a.patientId))===snap.patientId;});},[]);
    var apptTs=0; appts.forEach(function(a){apptTs=Math.max(apptTs,timeValue(a&& (a.athenaImportedAt||a.syncReceivedAt||a.updated_at||a.created_at)));});
    var coverage=source.athenaProfileCoverage||{}, coverageTs=timeValue(coverage.capturedAt||source.athenaChartImportedAt||(source.athenaChartSnapshot&&source.athenaChartSnapshot.capturedAt));
    var chartOk=coverage.complete===true&&coverage.exactIdentityVerified===true&&trim(coverage.patientId)===snap.patientId;
    var visits=Array.isArray(source.visits)?source.visits:[], exactVisits=visits.filter(function(v){return v&&v.identityVerified===true&&trim(v.identityBinding)===snap.patientId;});
    var fullVisits=exactVisits.filter(function(v){return v.fullDetail===true&&v.bodyComplete===true&&v.indexOnly!==true;}), visitTs=0;
    fullVisits.forEach(function(v){visitTs=Math.max(visitTs,timeValue(v.captured));});
    var verifyReceipt=getVerifyReceipt(snap.patientId), verifyState=verifyFreshness(snap,verifyReceipt);
    return {
      patient:snap,
      schedule:{linked:appts.length>0,count:appts.length,updatedAt:apptTs},
      chart:{verified:chartOk,capturedAt:chartOk?coverageTs:0,receipt:chartOk?coverage:null},
      history:{verified:fullVisits.length>0,count:fullVisits.length,totalExact:exactVisits.length,capturedAt:visitTs},
      verify:verifyReceipt,verifyFresh:verifyState.fresh,verifyReason:verifyState.reason,
      sends:sendCounts(snap.patientId)
    };
  }
  function state(){
    var log=getLog(), ps=patientStatus(), c=ps.sends;
    var errors=c.failed+c.uncertain;
    var sends=log.filter(function(e){return e&&e.dir==='push';});
    var last=log.length?log[log.length-1]:null;
    return { errors:errors, count:sends.length, last:last, total:log.length, patient:ps, verified:c.verified, pending:c.pending, failed:c.failed, uncertain:c.uncertain };
  }
  function chipHtml(){
    var st=state(), ps=st.patient, receipt=ps&&ps.verify;
    var cls='ok', txt;
    if(!ps||!ps.patient){cls='idle';txt='Athena · choose patient';}
    else if(ps.sends.failed||ps.sends.uncertain){cls='err';txt='Athena · '+(ps.sends.failed+ps.sends.uncertain)+' need review';}
  ...
  function render(){
    ...
    safe(function(){
      ...
      lastChipHtml=html;window.__mlsCard.setSyncSlot(html);bindChip();
    });
  }
  function bindChip(){
  ...
  function bridgeOnce(type,payload,respType,timeout,requestId){
    return new Promise(function(resolve){
      var done=false, tid=null;
      function finish(v){if(done)return;done=true;try{window.removeEventListener('message',on);}catch(e){}if(tid)clearTimeout(tid);resolve(v||null);}
      function on(ev){var d=ev&&ev.data;if(!d||d.source!=='mls-ext'||d.type!==respType)return;if(requestId&&trim(d.requestId)!==requestId)return;finish(d.resp||d);}
      try{window.addEventListener('message',on,false);}catch(e){}
      try{var msg={type:type,source:'mls-app',from:'mls-app'};Object.keys(payload||{}).forEach(function(k){msg[k]=payload[k];});if(requestId)msg.requestId=requestId;window.postMessage(msg,'*');}catch(e){finish({ok:false,error:'extension bridge unavailable'});return;}
      tid=setTimeout(function(){finish({__timeout:true,ok:false,error:'No read-only verification response arrived.'});},timeout||5000);
    });
  }
  function exactProbeContext(c){return !!(c&&trim(c.patientName)&&digits(c.dob)&&digits(c.mrn)&&digits(c.encounterId)&&trim(c.encounterUrl)&&trim(c.visitDate)&&trim(c.provider)&&trim(c.contextHash));}
  function verifyNow(){
    if(PASSIVE.verifying)return Promise.resolve(false);
    ...
    return bridgeOnce('mlsPing',{},'mlsPong',3000,'').then(function(pong){
      ...
      if((!resp||resp.ok!==true)&&/^(context-unverified|context-mismatch|patient-mismatch)$/.test(rzn)){
        ...
        return bridgeOnce('mlsAppSearchOpenPatient',{
          ...
          appointmentId:trim(expectedVisit.appointmentId),scheduleDate:dateKey(expectedVisit.visitDate),bootstrapIdentity:!!(trim(expectedVisit.appointmentId)&&dateKey(expectedVisit.visitDate)),
      ...
      if(!resp||resp.__timeout||resp.ok!==true||resp.readOnly!==true||resp.mode!=='probe'||resp.action!=='write_note'||resp.noAutomaticChaining!=='no-automatic-chaining')throw new Error(trim(resp&&(resp.error||resp.message||resp.reason))||'Athena did not return a read-only exact-patient receipt.');
      if(!sameIdentity(frozen,ctx)||!exactProbeContext(ctx))throw new Error('Athena returned a different or incomplete patient/encounter identity. Nothing was changed.');
      var receipt={
        kind:'athena-active-patient-verification',patientId:frozen.patientId,patientName:frozen.name,patientDob:frozen.dob,patientMrn:frozen.mrn,
        verifiedAt:new Date().toISOString(),readOnly:true,source:'explicit-verify',extensionVersion:PASSIVE.version||'',
        localContext:localVisitContext(frozen.patientId),
        context:{patientName:trim(ctx.patientName),dob:trim(ctx.dob),mrn:trim(ctx.mrn),encounterId:trim(ctx.encounterId),visitDate:trim(ctx.visitDate),provider:trim(ctx.provider),contextHash:trim(ctx.contextHash)}
      };
      saveVerifyReceipt(receipt);PASSIVE.verifyError='';return true;
    }).catch(function(err){PASSIVE.verifyError=trim(err&&err.message)||'Read-only verification failed.';return false;}).then(function(ok){PASSIVE.verifying=false;render();refreshOpenPop();return ok;});
  }
  function masked(v){var d=digits(v);return d?('••••'+d.slice(-4)):'not recorded';}
  function stamp(ts,missing){return ts?(fmtTime(ts)+' · '+timeAgo(ts)):(missing||'timestamp not recorded');}
  function statusRow(label,value,kind){return '<div class="mls-sp-check '+esc(kind||'')+'"><b>'+esc(label)+'</b><span>'+esc(value)+'</span></div>';}
  function matchingScheduleReceipt(p){
    var sr=PASSIVE.schedule;if(!p||!sr||!sr.complete)return null;
    var hit=(sr.rows||[]).some(function(r){return normName(r&&r.name)===normName(p.name)&&digits(r&&r.dob)===digits(p.dob)&&(!digits(p.mrn)||!digits(r&&(r.mrn||r.athenaId))||digits(r&&(r.mrn||r.athenaId))===digits(p.mrn));});
    return hit?sr:null;
  }
  function fillPop(pop){
    if(!pop)return;
    var ps=patientStatus(), p=ps.patient, sr=p&&matchingScheduleReceipt(p), recent=p?patientLog(p.patientId).slice().reverse().slice(0,5):[];
    var rows=recent.length?recent.map(function(e){var s=sendStatus(e),ic=s==='verified'?'✓':(s==='pending'?'…':(s==='failed'?'×':'?'));return '<div class="mls-sp-row"><span class="mls-sp-ic">'+ic+'</span><span class="mls-sp-main"><b>'+esc(e.label||e.target||'Athena action')+'</b><span class="mls-sp-t">'+esc(s+' · '+fmtTime(e.ts))+'</span></span></div>';}).join(''):'<div class="mls-sp-empty">No send attempts recorded for this patient.</div>';
    var body='<div class="mls-sp-head"><b>Active-patient Athena status</b><span class="mls-sp-mode">'+esc(connMode())+'</span></div>';
    if(!p){body+='<div class="mls-sp-empty">Choose a patient to see exact schedule, chart, history, and send receipts.</div>';pop.innerHTML=body;return;}
    body+='<div class="mls-sp-patient"><b>'+esc(p.name||'Unnamed patient')+'</b><span>MLS ID '+esc(p.patientId)+' · DOB '+esc(p.dob||'not recorded')+' · MRN '+esc(masked(p.mrn))+'</span></div>';
    body+='<div class="mls-sp-checks">'
      +statusRow('Schedule',ps.schedule.linked?(ps.schedule.count+' linked appointment'+(ps.schedule.count===1?'':'s')+' by immutable MLS patient ID · '+(sr?('Athena day receipt '+stamp(sr.observedAt)):('calendar freshness '+stamp(ps.schedule.updatedAt,'no fresh Athena receipt observed')))):'No appointment linked to this immutable patient ID',ps.schedule.linked?'ok':'idle')
      +statusRow('Chart',ps.chart.verified?('Exact six-card coverage receipt · '+stamp(ps.chart.capturedAt)):'No exact-patient chart coverage receipt',''+(ps.chart.verified?'ok':'idle'))
      +statusRow('History',ps.history.verified?(ps.history.count+' body-complete visit'+(ps.history.count===1?'':'s')+' · '+stamp(ps.history.capturedAt)):(ps.history.totalExact?('Only '+ps.history.totalExact+' exact index row(s); full bodies not verified'):'No exact-patient full-visit receipt'),ps.history.verified?'ok':'idle')
      +statusRow('Open Athena encounter',ps.verify&&ps.verifyFresh?('Verified read-only '+stamp(timeValue(ps.verify.verifiedAt))):((PASSIVE.verifyError||ps.verifyReason||'Not verified yet')+(PASSIVE.verifying?' Working…':'')),ps.verifyFresh?'ok':((PASSIVE.verifyError||ps.verify)?'err':'idle'))
      +statusRow('Extension / session',PASSIVE.pongAt?('Extension '+(PASSIVE.version?('v'+PASSIVE.version+' '):'')+'responded '+timeAgo(PASSIVE.pongAt)+'; Athena session is proven only by Verify now'):(window.__mlsExtReportedVersion?('Extension v'+window.__mlsExtReportedVersion+' reported installed; readiness not checked yet'):'No passive extension response observed'),PASSIVE.pongAt?'ok':'idle')
      +'</div>';
  ...
  function refreshOpenPop(){fillPop(document.getElementById('mlsSyncPop'));}
  function togglePop(anchor){
    var ex=document.getElementById('mlsSyncPop'); if(ex){ ex.remove(); return; }
    var pop=document.createElement('div'); pop.id='mlsSyncPop';fillPop(pop);document.body.appendChild(pop);
    var r=anchor.getBoundingClientRect();pop.style.top=(r.bottom+6+window.scrollY)+'px';pop.style.left=Math.max(8,Math.min(r.left+window.scrollX,window.innerWidth-430))+'px';
  ...
  var ACTIONS=[
    {re:/pushEntireVisitToAthena/, label:'Athena review opened', target:'visit', dir:'review'},
    {re:/pushSuperbillToAthena/, label:'Superbill review opened', target:'superbill', dir:'review'},
    {re:/pushHistoryNoteToAthena/, label:'Saved-note review opened', target:'note', dir:'review'},
    {re:/copyForEMR/, label:'Copied note text', target:'copy', dir:'local'},
  ...
  function onClickCapture(e){
    safe(function(){
      ...
      while(el && hops<5){
        ...
        if(oc){ for(var i=0;i<ACTIONS.length;i++){ if(ACTIONS[i].re.test(oc)){ if(ACTIONS[i].dir==='review'||ACTIONS[i].dir==='local')return; mark({label:ACTIONS[i].label, target:ACTIONS[i].target, dir:ACTIONS[i].dir, status:'uncertain', receiptVerified:false, detail:'UI action observed; no durable receipt was available.'}); return; } } }
        el=el.parentElement; hops++;
      }
    });
  }
  function actionLabel(action){return ({write_note:'Reviewed note write',stage_billing:'Billing code staging',save_draft:'Athena draft save',sign_encounter:'Athena sign & save',place_order:'Reviewed order placement'})[trim(action)]||'Athena action';}
  function verifiedResult(action,r){
    if(action==='write_note')return r.written===true&&r.verified===true&&r.draftVerified===true;
    if(action==='stage_billing')return r.staged===true||r.verified===true;
    if(action==='save_draft')return r.saved===true||r.persisted===true||r.serverVerified===true||r.verified===true;
    if(action==='sign_encounter')return r.signed===true;
    if(action==='place_order')return r.verified===true&&(r.orderPlaced===true||r.alreadyPresent===true);
    return false;
  }
  function classifyResult(action,r){
    r=r||{};
    if(r.__timeout||r.partialMutation===true||r.reason==='outcome-uncertain')return'uncertain';
    if(r.ok!==true)return(r.attempted===true?'uncertain':'failed');
    return verifiedResult(action,r)?'verified':'uncertain';
  }
  function onBridgeMessage(ev){
    var d=ev&&ev.data;if(!d||!d.type)return;
    if(d.source==='mls-ext'&&d.type==='mlsPong'){
      PASSIVE.pongAt=Date.now();PASSIVE.version=trim(d.version||d.extVersion||(d.resp&&(d.resp.version||d.resp.extVersion))||window.__mlsExtReportedVersion);render();refreshOpenPop();return;
    }
    if(d.source==='mls-ext'&&d.type==='mlsAppScheduleResult'){
      var sr=d.resp||d, receipt=sr&&sr.receipt;
      if(receipt&&receipt.complete===true)PASSIVE.schedule={complete:true,observedAt:Date.now(),day:trim(sr.schedDate),rows:Array.isArray(sr.appts)?sr.appts.slice():[],receipt:{expectedCount:Number(receipt.expectedCount||0),parsedCount:Number(receipt.parsedCount||0),authoritativeEmpty:receipt.authoritativeEmpty===true}};
      refreshOpenPop();return;
    }
    if(d.source==='mls-app'&&d.type==='mlsAppAthenaActionV2'&&trim(d.mode)==='execute'){
      var req=trim(d.requestId), p=d.expectedPatient||d.patient||{}, pid=trim(p.patientId||p.id);
      if(!req||!pid)return;
      PASSIVE.pending[req]={requestId:req,patientId:pid,patient:trim(p.name),action:trim(d.action),label:actionLabel(trim(d.action)),dir:'push',status:'pending',receiptVerified:false,ts:Date.now()};
      mark(PASSIVE.pending[req]);return;
    }
    if(d.source==='mls-ext'&&d.type==='mlsAppAthenaActionV2Result'){
      var rid=trim(d.requestId), pending=PASSIVE.pending[rid];if(!pending)return;
      var resp=d.resp||d, status=classifyResult(pending.action,resp);
      mark(Object.assign({},pending,{status:status,receiptVerified:status==='verified',completedAt:Date.now(),detail:trim(resp.error||resp.message||resp.reason)}));delete PASSIVE.pending[rid];
    }
  }
  ...
  function init(){
    ...
    render();
    if(!renderTimer)renderTimer=setInterval(render,4000);
  ...
  function injectCss(){
    ...
    s.textContent=
      ...
      +'#mlsSyncPop{position:absolute;z-index:100000;width:min(420px,calc(100vw - 16px));background:var(--card,#fff);border:1px solid var(--line,#e6e9ef);border-radius:12px;box-shadow:0 16px 40px rgba(15,28,46,.22);font-size:13px;overflow:hidden;}'
      ...
      +'#mlsSyncPop .mls-sp-patient{display:flex;flex-direction:column;gap:3px;padding:10px 13px;background:#f4f9f6;border-bottom:1px solid var(--line,#e6e9ef);color:#1f3e31;}#mlsSyncPop .mls-sp-patient span{font-size:11px;color:#536b60;overflow-wrap:anywhere;}'
      +'#mlsSyncPop .mls-sp-checks{padding:6px 13px;}#mlsSyncPop .mls-sp-check{display:grid;grid-template-columns:128px 1fr;gap:9px;padding:6px 0;border-bottom:1px solid #eef1ef;}#mlsSyncPop .mls-sp-check:last-child{border-bottom:0;}#mlsSyncPop .mls-sp-check b{font-size:11.5px;}#mlsSyncPop .mls-sp-check span{font-size:11.5px;color:#53615a;line-height:1.35;}#mlsSyncPop .mls-sp-check.ok b{color:#17633f;}#mlsSyncPop .mls-sp-check.err b{color:#a52222;}'
      +'#mlsSyncPop .mls-sp-verify{display:block;width:calc(100% - 26px);margin:10px 13px 5px;border:0;border-radius:8px;background:#204f3c;color:#fff;padding:9px 12px;font:700 12.5px system-ui;cursor:pointer;}#mlsSyncPop .mls-sp-verify:disabled{opacity:.6;cursor:wait;}#mlsSyncPop .mls-sp-safe{padding:0 13px 9px;color:#63736b;font-size:10.5px;line-height:1.4;}'
  ...
  window.__mlsSync={ mark:mark, getLog:getLog, state:state, patientStatus:patientStatus, verifyNow:verifyNow, render:render, _sameIdentity:sameIdentity, _verifyFreshness:verifyFreshness, _classifyResult:classifyResult };
})();
...
(function(){try{if(document.getElementById('mlsOutcomeStudyLoader'))return;var s=document.createElement('script');s.id='mlsOutcomeStudyLoader';s.src='mls-outcome-study.staging.js?v=20260718stglib1';s.async=false;(document.head||document.documentElement).appendChild(s);}catch(e){}})();
...
;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_opnote_prep.js"]'))return;var s=document.createElement('script');s.src='feat_mls_opnote_prep.js?v=20260731opnp181';s.setAttribute('data-mls-asset','feat_mls_opnote_prep.js');s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})();
...
;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_opnote_integrity.js"]'))return;var s=document.createElement('script');s.src='feat_mls_opnote_integrity.js?v='+(window.__MLS_AV||Date.now());s.async=false;s.setAttribute('data-mls-asset','feat_mls_opnote_integrity.js');(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})();
;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_template_library.js"]'))return;var s=document.createElement('script');s.src='feat_mls_template_library.js?v='+(window.__MLS_AV||Date.now());s.setAttribute('data-mls-asset','feat_mls_template_library.js');s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})();

/* ---- loader: mls-procedure-report (Analysis › 📊 Procedure Report: counts by type, Office/ASC place-of-service, RVU/$ totals, PDF/CSV) ---- */
(function(){try{if(window.__mlsProcReportLoader)return;window.__mlsProcReportLoader=1;var s=document.createElement('script');s.src='mls-procedure-report.staging.js?v=20260718stglib1';s.async=true;(document.head||document.documentElement).appendChild(s);}catch(e){}})();
...
;(function(){try{var A='feat_mls_topbar_unify.js',V='1.0.10',api=window.__mlsTopbar,old=document.querySelector('script[data-mls-asset="'+A+'"]');if(api&&api.installed&&api.version===V)return;if(old){if(!api)return;old.setAttribute('data-mls-retired-asset',A);old.removeAttribute('data-mls-asset');}var s=document.createElement('script');s.src='/feat_mls_topbar_unify.js?v=20260722tb111';s.setAttribute('data-mls-asset',A);s.async=false;(document.head||document.documentElement).appendChild(s);}catch(e){}})();
...
;(function(){try{var A='feat_mls_redesign.js',V='3.2.4',api=window.__mlsRedesign,old=document.querySelector('script[data-mls-asset="'+A+'"]');if(api&&api.installed&&api.version===V)return;if(old){if(!api)return;old.setAttribute('data-mls-retired-asset',A);old.removeAttribute('data-mls-asset');}var s=document.createElement('script');s.src=A+'?v='+(window.__MLS_AV||Date.now());s.setAttribute('data-mls-asset',A);s.async=false;(document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* MLSscribe 2026 reskin: version-aware same-document upgrade, additive and reversible. */
;(function(){try{var A='feat_mls_command_palette.js',V='cpal-1.0.4',api=window.__mlsCmdPalette,old=document.querySelector('script[data-mls-asset="'+A+'"]');if(api&&api.installed&&api.version===V)return;if(old){if(!api)return;old.setAttribute('data-mls-retired-asset',A);old.removeAttribute('data-mls-asset');}var s=document.createElement('script');s.src='feat_mls_command_palette.js?v=20260808cmd106perf2';s.setAttribute('data-mls-asset',A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* STAGING parity: version-aware Ctrl+K converges on canonical Find (cpal-1.0.4). */

;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_visit_exact.js"]'))return;var s=document.createElement('script');s.src='feat_mls_visit_exact.js?v=20260726dkn1';s.setAttribute('data-mls-asset','feat_mls_visit_exact.js');s.async=false;(document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* MLSscribe Visit design-exact rebuild (STAGING ONLY) — additive, reversible (delete this line + feat_mls_visit_exact.js) */

;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_header_exact.js"]'))return;var s=document.createElement('script');s.src='feat_mls_header_exact.js?v=20260624hx5';s.setAttribute('data-mls-asset','feat_mls_header_exact.js');s.async=false;(document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* MLSscribe header/nav design-exact (STAGING ONLY) — additive, reversible (delete this line + feat_mls_header_exact.js) */
...
;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_history_exact.js"]'))return;var s=document.createElement('script');s.src='feat_mls_history_exact.js?v=20260726dkn1';s.setAttribute('data-mls-asset','feat_mls_history_exact.js');s.async=false;(document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* MLSscribe feat_mls_history_exact.js (STAGING ONLY) — additive, reversible */

;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_orders_exact.js"]'))return;var s=document.createElement('script');s.src='feat_mls_orders_exact.js?v=20260726dkn1';s.setAttribute('data-mls-asset','feat_mls_orders_exact.js');s.async=false;(document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* MLSscribe feat_mls_orders_exact.js (STAGING ONLY) — additive, reversible */

;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_recs_exact.js"]'))return;var s=document.createElement('script');s.src='feat_mls_recs_exact.js?v=20260726dkn1';s.setAttribute('data-mls-asset','feat_mls_recs_exact.js');s.async=false;(document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* MLSscribe feat_mls_recs_exact.js (STAGING ONLY) — additive, reversible */

;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_legal_exact.js"]'))return;var s=document.createElement('script');s.src='feat_mls_legal_exact.js?v=20260726dkn1';s.setAttribute('data-mls-asset','feat_mls_legal_exact.js');s.async=false;(document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* MLSscribe feat_mls_legal_exact.js (STAGING ONLY) — additive, reversible */

;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_team_exact.js"]'))return;var s=document.createElement('script');s.src='feat_mls_team_exact.js?v=20260726dkn1';s.setAttribute('data-mls-asset','feat_mls_team_exact.js');s.async=false;(document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* MLSscribe feat_mls_team_exact.js (STAGING ONLY) — additive, reversible */

;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_analysis_exact.js"]'))return;var s=document.createElement('script');s.src='feat_mls_analysis_exact.js?v=20260722ax7';s.setAttribute('data-mls-asset','feat_mls_analysis_exact.js');s.async=false;(document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* MLSscribe feat_mls_analysis_exact.js (STAGING ONLY) — additive, reversible */
...
;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_notetools_exact.js"]'))return;var s=document.createElement('script');s.src='feat_mls_notetools_exact.js?v=20260624nt1';s.setAttribute('data-mls-asset','feat_mls_notetools_exact.js');s.async=false;(document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* MLSscribe feat_mls_notetools_exact.js (STAGING) - note+tools legibility, additive, reversible */

;(function(){try{if(document.querySelector('script[data-mls-asset="mls-template-stdline.js"]'))return;var s=document.createElement('script');s.src='/mls-template-stdline.js?v='+Date.now();s.setAttribute('data-mls-asset','mls-template-stdline.js');document.head.appendChild(s);}catch(e){}})(); /* FIX 3: mounts #mls-stdline-section into #templatesModal + wraps openTemplates (was prod-only; add to staging so the Templates AI add-a-line section appears) */
;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_widgetinsert.js"]'))return;var s=document.createElement('script');s.src='feat_mls_widgetinsert.js?v=20260624wi2';s.setAttribute('data-mls-asset','feat_mls_widgetinsert.js');s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* FIX 2: robust custom-widget Add-to-note (body/mirror fallback) + surface generated widgets (STAGING) - additive, reversible */
;(function(){try{var A='feat_mls_datalink_exact.js';if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement('script');s.src=A+'?v='+(window.__MLS_AV||Date.now());s.setAttribute('data-mls-asset',A);s.async=false;(document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* MLSscribe link-1.1.0 event-driven cross-surface data link (STAGING), additive and reversible */

;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_assistant_exact.js"]'))return;var s=document.createElement('script');s.src='feat_mls_assistant_exact.js?v=20260808asst220perf1';s.setAttribute('data-mls-asset','feat_mls_assistant_exact.js');s.async=false;(document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* MLSscribe feat_mls_assistant_exact.js (STAGING) - one honest assistant panel, additive reversible */
;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_schedimport_exact.js"]'))return;var s=document.createElement('script');s.src='feat_mls_schedimport_exact.js?v='+(window.__MLS_AV||Date.now());s.setAttribute('data-mls-asset','feat_mls_schedimport_exact.js');s.async=false;(document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* MLSscribe feat_mls_schedimport_exact.js (STAGING si-1.7.22) - exact provider/day/month identity + responsive history organization + cooperative patient persistence */

/* STAGING Staff Prep parity. Menu owns discovery; this receiver opens one
   explicit, read-only day/month pull surface backed by the exact importer.
   Opening the dialog never starts a pull and never writes to Athena. */
;(function(){
  'use strict';
  try{if(window.__mlsStagingStaffPrep&&window.__mlsStagingStaffPrep.installed)return;}catch(e){return;}
  var VERSION='ssp-1.0.0',OV='mlsStagingStaffPrep',STYLE='mlsStagingStaffPrepStyle',busy=false;
  ...
  function setStatus(message,kind){var el=byId('mlsSspStatus');if(!el)return;el.textContent=String(message||'');el.setAttribute('data-kind',kind||'');}
  function setBusy(on){busy=!!on;['mlsSspPullDay','mlsSspPullMonth'].forEach(function(id){var button=byId(id);if(button)button.disabled=busy;});}
  function providerRows(){
    var roster=safe(function(){return window.__mlsProviderRoster;},null);
    var rows=roster&&typeof roster.list==='function'?safe(function(){return roster.list();},[]):[];
    return Array.isArray(rows)?rows:[];
  }
  function populateProviders(){
    var select=byId('mlsSspProvider');if(!select)return;
    var prior=select.value||'all';select.innerHTML='';
    var all=document.createElement('option');all.value='all';all.textContent='All providers';select.appendChild(all);
    providerRows().forEach(function(row){
      if(!row||!row.stableKey||!row.name)return;
      var option=document.createElement('option');option.value='pv:'+encodeURIComponent(String(row.stableKey));option.textContent=String(row.name);select.appendChild(option);
    });
    var exists=false;for(var i=0;i<select.options.length;i++)if(select.options[i].value===prior){exists=true;break;}select.value=exists?prior:'all';
    var receipt=safe(function(){return window.__mlsProviderRoster&&window.__mlsProviderRoster.getReceipt&&window.__mlsProviderRoster.getReceipt();},null);
    setStatus(receipt&&receipt.complete===true
      ?'Verified Athena provider roster ready. Choose a day or month; nothing runs until you press Pull.'
      :'Choose All providers for a day pull. A verified Athena provider roster is required for clinician-specific or month-wide pulls.','');
  }
  function selectedProvider(){
    var select=byId('mlsSspProvider'),value=select?String(select.value||'all'):'all';
    if(value==='all')return 'all';
    var roster=safe(function(){return window.__mlsProviderRoster;},null);
    var row=roster&&typeof roster.resolve==='function'?safe(function(){return roster.resolve(value);},null):null;
    if(!row||!row.stableKey||!row.name)throw new Error('That clinician is not uniquely verified in the current Athena roster. Pull one day for All providers, then choose the clinician again.');
    var receipt=typeof roster.getReceipt==='function'?safe(function(){return roster.getReceipt();},null):null;
    return{id:String(row.id||''),stableKey:String(row.stableKey),raw:String(row.raw||row.name),name:String(row.name),rosterVerified:!!(receipt&&receipt.complete===true)};
  }
  function summarize(result,label){
    result=result||{};
    if(result.ok!==true||result.complete!==true){setStatus(String(result.error||result.message||result.reason||('The '+label+' pull did not complete. Nothing was written to Athena.')),'err');return;}
    setStatus(String(result.message||('The '+label+' schedule/history pull completed. Review the imported patients before using their history.')),'ok');
  }
  function run(kind){
    if(busy)return;
    var si=safe(function(){return window.__mlsSI;},null),method=kind==='month'?'pullMonth':'pull';
    if(!si||typeof si[method]!=='function'){setStatus('The exact Athena schedule reader is still loading. Close Staff prep, wait a moment, and try again.','err');return;}
    var provider;try{provider=selectedProvider();}catch(error){setStatus(error.message,'err');return;}
    var input=byId(kind==='month'?'mlsSspMonth':'mlsSspDay'),value=input&&input.value;
    if(!value){setStatus('Choose a '+kind+' first.','err');return;}
    setBusy(true);setStatus('Reading the selected '+kind+' from Athena and verifying available patient histories…','busy');
    var opts={provider:provider,includeHistory:true,onStatus:function(message,state){setStatus(message,state||'busy');}};opts[kind==='month'?'month':'date']=value;
    Promise.resolve().then(function(){return si[method](opts);}).then(function(result){summarize(result,kind);},function(error){setStatus(String(error&&error.message||('The '+kind+' pull failed. Nothing was written to Athena.')),'err');}).then(function(){setBusy(false);});
  }
  function close(){var ov=byId(OV);if(ov)ov.style.display='none';}
  function ensure(){
    var ov=byId(OV);if(ov)return ov;
    var style=document.createElement('style');style.id=STYLE;
    style.textContent='#'+OV+'{position:fixed;inset:0;z-index:2147482500;background:rgba(26,33,28,.46);display:none;align-items:flex-start;justify-content:center;padding:9vh 16px 20px}#'+OV+' .ssp-card{width:min(680px,100%);background:var(--card,#fff);color:var(--ink,#1A211C);border:1px solid var(--line,#E7E5DD);border-radius:16px;box-shadow:0 24px 70px rgba(20,33,28,.28);padding:20px}#'+OV+' .ssp-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:16px}#'+OV+' h2{margin:0 0 4px;font:700 20px/1.25 inherit}#'+OV+' p{margin:0;color:var(--muted,#68736B);font-size:13px;line-height:1.5}#'+OV+' .ssp-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:16px 0}#'+OV+' label{display:flex;flex-direction:column;gap:6px;font-size:12px;font-weight:700}#'+OV+' input,#'+OV+' select{height:40px;border:1px solid var(--line,#D6D2C6);border-radius:9px;background:var(--field-bg,#FCFBF8);color:inherit;padding:0 10px;font:500 13px inherit}#'+OV+' .ssp-actions{display:flex;gap:9px;flex-wrap:wrap}#'+OV+' button{min-height:40px;border:1px solid var(--line,#D6D2C6);border-radius:9px;background:var(--card,#fff);color:inherit;padding:0 14px;font:700 13px inherit;cursor:pointer}#'+OV+' button.ssp-primary{background:var(--brand-dk,#204034);border-color:var(--brand-dk,#204034);color:#fff}#'+OV+' button:disabled{opacity:.55;cursor:wait}#mlsSspStatus{margin-top:14px;padding:11px 12px;border-radius:9px;background:var(--soft,#F4F2EC);font-size:12.5px;line-height:1.45}#mlsSspStatus[data-kind="err"]{color:#9A2929;background:#FFF1F1}#mlsSspStatus[data-kind="ok"]{color:#1F6B45;background:#ECF8F1}@media(max-width:620px){#'+OV+'{padding:12px;align-items:center}#'+OV+' .ssp-grid{grid-template-columns:1fr}}';
    (document.head||document.documentElement).appendChild(style);
    ov=document.createElement('div');ov.id=OV;ov.setAttribute('role','dialog');ov.setAttribute('aria-modal','true');ov.setAttribute('aria-labelledby','mlsSspTitle');
    ov.innerHTML='<section class="ssp-card"><div class="ssp-head"><div><h2 id="mlsSspTitle">Staff prep</h2><p>Pull an exact Athena day or month, including available verified history. This is read-only and never opens, changes, signs, or sends an Athena encounter.</p></div><button type="button" id="mlsSspClose" aria-label="Close Staff prep">Close</button></div><div class="ssp-grid"><label>Provider<select id="mlsSspProvider"></select></label><label>Day<input id="mlsSspDay" type="date"></label><label>Month<input id="mlsSspMonth" type="month"></label></div><div class="ssp-actions"><button type="button" class="ssp-primary" id="mlsSspPullDay">Pull this day</button><button type="button" class="ssp-primary" id="mlsSspPullMonth">Pull this month</button><button type="button" id="mlsSspPrepNotes">Prep op notes for day</button></div><div id="mlsSspStatus" role="status" aria-live="polite"></div></section>';
    document.body.appendChild(ov);
    byId('mlsSspClose').addEventListener('click',close);
    byId('mlsSspPullDay').addEventListener('click',function(){run('day');});
    byId('mlsSspPullMonth').addEventListener('click',function(){run('month');});
    byId('mlsSspPrepNotes').addEventListener('click',function(){var day=byId('mlsSspDay');if(day&&day.value&&typeof window.openOpPrep==='function'){close();window.openOpPrep(day.value);}else setStatus('Choose a day first; the op-note prep tool is still loading if this continues.','err');});
    ov.addEventListener('mousedown',function(event){if(event.target===ov)close();});return ov;
  }
  function open(){
    var ov=ensure();if(!ov)return false;
    var day=byId('mlsSspDay'),month=byId('mlsSspMonth');if(day&&!day.value)day.value=dayKey();if(month&&!month.value)month.value=monthKey();
    populateProviders();ov.style.display='flex';setTimeout(function(){var first=byId('mlsSspProvider');if(first)first.focus();},0);return true;
  }
  function onRequest(event){
    var detail=event&&event.detail;if(!detail||detail.source!=='mls-topbar-menu'||!detail.requestId)return;
    if(!open())return;
    safe(function(){window.dispatchEvent(new CustomEvent('mls:menu-staff-prep-opened',{detail:{source:'mls-staging-staff-prep',requestId:detail.requestId}}));});
  }
  function onKey(event){if(event&&(event.key==='Escape'||event.key==='Esc')){var ov=byId(OV);if(ov&&ov.style.display!=='none'){event.preventDefault();event.stopImmediatePropagation();close();}}}
  function revert(){
    safe(function(){window.removeEventListener('mls:menu-staff-prep-request',onRequest);});safe(function(){document.removeEventListener('keydown',onKey,true);});
    [OV,STYLE].forEach(function(id){var node=byId(id);if(node&&node.parentNode)node.parentNode.removeChild(node);});try{window.__mlsStagingStaffPrep.installed=false;}catch(e){}
  }
  window.addEventListener('mls:menu-staff-prep-request',onRequest);document.addEventListener('keydown',onKey,true);
  window.__mlsStagingStaffPrep={installed:true,version:VERSION,revert:revert};
})();
...
;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_calbox_uniform.js"]'))return;var s=document.createElement('script');s.src='feat_mls_calbox_uniform.js?v=20260727cb110';s.setAttribute('data-mls-asset','feat_mls_calbox_uniform.js');s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* item9: uniform blue calendar Week/Day appt boxes (additive reversible) */
...
;(function(){try{if(window.__mlsCanonicalOpNoteFillRequested||(window.__mlsOpNoteFill&&window.__mlsOpNoteFill.installed))return;if(document.querySelector('script[data-mls-asset="feat_mls_opnote_fillblank.js"]'))return;var s=document.createElement('script');s.src='feat_mls_opnote_fillblank.js?v=20260625of1';s.setAttribute('data-mls-asset','feat_mls_opnote_fillblank.js');s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* fallback only when the canonical op-note Fields owner was not requested */
...
;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_copilot_unify.js"]'))return;var s=document.createElement('script');s.src='feat_mls_copilot_unify.js?v='+(window.__MLS_AV||Date.now());s.setAttribute('data-mls-asset','feat_mls_copilot_unify.js');s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* unify-1.2.0 STAGING parity: one Copilot conversation plus O(1) active-id no-patient hints */
;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_asst_fix.js"]'))return;var s=document.createElement('script');s.src='feat_mls_asst_fix.js?v='+(window.__MLS_AV||Date.now());s.setAttribute('data-mls-asset','feat_mls_asst_fix.js');s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* item19: MLS Assistant fixes + canonical provider roster gate -- additive, reversible (window.__mlsAsstFix.revert()) */
;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_copilot_voice_v2.js"]'))return;var s=document.createElement('script');s.src='feat_mls_copilot_voice_v2.js?v=20260714cv2115';s.setAttribute('data-mls-asset','feat_mls_copilot_voice_v2.js');s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* STAGING parity: MLS Copilot Voice v2 */
...
;(function(){try{if(document.querySelector('script[data-mls-asset="feat_mls_checker.js"]'))return;var s=document.createElement('script');s.src='feat_mls_checker.js?v=20260808chk3053';s.setAttribute('data-mls-asset','feat_mls_checker.js');s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* item21: MLS Checker -- honest self-diagnostic registry of named checks (pass/fail + code + cause + fix) surfaced in the MLS Assistant -- additive, reversible (window.__mlsChecker.revert()) */
...
;(function(){try{var A='feat_mls_upnow_activeselect.js';if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement('script');s.src=A+'?v=20260808uas5perf1';s.setAttribute('data-mls-asset',A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* item25: align active patient with auto-pulled up-now patient on load so header/Outcomes/After-visit match the banner/NEXT UP highlight (guarded: skips during capture/recording and deliberate chart switch) -- additive, reversible (window.__mlsUpNowActiveSelect.revert()) */
...
;(function(){try{var A='feat_mls_active_patient_sync.js';if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement('script');s.src=A+'?v='+(window.__MLS_AV||Date.now());s.setAttribute('data-mls-asset',A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* aps-1.2.2: exact switch/update sync plus a 15s id/field-only structural backstop (additive, reversible) */

;(function(){try{var A="feat_mls_schedpull_honest.js";if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement("script");s.src=A+"?v=20260625honest1";s.setAttribute("data-mls-asset",A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* item28: kill residual false "No new appointments were imported..." status line (honest status only) -- additive, reversible (window.__mlsSchedPullHonest.revert()) */
...
;(function(){try{var A="feat_mls_dob_format_unify.js";if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement("script");s.src=A+"?v=20260626dob1";s.setAttribute("data-mls-asset",A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* item38 (STAGING): unify displayed patient DOB format to canonical MM/DD/YYYY across all card surfaces to match context-bar item35; display-only, no patient-data mutation -- additive, reversible (window.__mlsDobFormatUnify.revert()) */

;(function(){try{var A="feat_mls_calendar_polish.js";if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement("script");s.src=A+"?v="+(window.__MLS_AV||Date.now());s.setAttribute("data-mls-asset",A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* item67 (STAGING): canonical verified provider roster + exact provider-day history pull */

;(function(){try{var A="feat_mls_caldedupe_render.js";if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement("script");s.src=A+"?v=20260727dd110";s.setAttribute("data-mls-asset",A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* item68 (STAGING): calendar/agenda duplicate-appointment fix -- idempotent read-layer dedupe of window._calAppts by stable identity (name|date|time|provider) before each render; display-only, never deletes records (additive, reversible: window.__mlsCalDedupe.revert()) */
...
;(function(){try{var A="feat_mls_pervisit_unify.js";if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement("script");s.src=A+"?v=20260725pvu1c2";s.setAttribute("data-mls-asset",A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* item70: unify copy-every-visit + per-visit records into one coherent timeline/detail surface + verify single model; additive, reversible: window.__mlsPerVisitUnify.revert() */

;(function(){try{['feat_visit_detail.js','feat_visit_note_detail.js','feat_visit_history_ext.js'].forEach(function(f){if(document.querySelector('script[data-mls-asset="'+f+'"]'))return;var s=document.createElement("script");s.src=f+"?v=20260714rich3";s.async=false;s.setAttribute("data-mls-asset",f);(document.body||document.head||document.documentElement).appendChild(s);});}catch(e){}})(); /* item71: wire rich per-visit detail UI to STAGING -- polished modal detail (__mlsVisitDetail) + note->visit bridge (__mlsVisitNoteDetail) + filtered extended timeline (__mlsVisitHistoryExt); ordered detail->note->ext; reuses single __mlsVisitModel; additive, reversible: each exposes .revert() */

;(function(){try{var A="feat_mls_dayprogress.js";if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement("script");s.src=A+"?v=20260727dp7";s.setAttribute("data-mls-asset",A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* item72 (STAGING): Day Progress meter in the persistent patient bar -- seen X/Y today + Next-up patient (one shared schedule/seen/now source of truth, click reuses _heroPickPatient), visible on every clinical view (additive, reversible: window.__mlsDayProgress.revert()) */
;(function(){try{var A="feat_mls_recentpts.js";if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement("script");s.src=A+"?v="+(window.__MLS_AV||Date.now());s.setAttribute("data-mls-asset",A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* item73 rp-2.3.0 (STAGING): exact-event, hidden-aware, route-fast recent-patient quick-switcher (additive, reversible: window.__mlsRecentPts.revert()) */
;(function(){try{var A="feat_mls_ptsnapshot.js";if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement("script");s.src=A+"?v=20260727ps3";s.setAttribute("data-mls-asset",A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* item74 (STAGING): Patient Snapshot chip in the patient bar -- one-click at-a-glance quick history (reuses _patientQuickHistory) + Open chart / Start visit shortcuts from any view (additive, reversible: window.__mlsPtSnapshot.revert()) */
...
;(function(){try{var A="feat_mls_allergy_alert.js";if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement("script");s.src=A+"?v=20260727aa2";s.setAttribute("data-mls-asset",A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* item76 (STAGING): Allergy safety chip in the patient bar -- always-visible drug-allergy alert (count + click for full reactions) sourced from the active patient record; display-only, never writes records (additive, reversible: window.__mlsAllergyAlert.revert()) */
;(function(){try{var A="feat_mls_agenda_popover.js";if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement("script");s.src=A+"?v=20260727ag-stable5";s.setAttribute("data-mls-asset",A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* item77 (STAGING): Today's agenda popover in the patient bar -- full ordered schedule (seen/up-now/upcoming) sharing Day-Progress's source of truth; click a row to load that patient via _heroPickPatient; navigation-only (additive, reversible: window.__mlsAgenda.revert()) */
...
;(function(){try{var A="feat_mls_outcome_everyvisit.js";if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement("script");s.src=A+"?v=20260630ev1";s.setAttribute("data-mls-asset",A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* item79 (STAGING): Outcome Study -- pull EVERY visit per cohort patient via the existing copy-every-visit machinery (__mlsCopyVisits/__mlsVisitModel) and auto-fill baseline + all follow-up timepoints from the full visit history through the study's own extract knob + _buildPatientStudy; manual spreadsheet stays as fallback; athenaOne read-only, never fabricates scores (additive, reversible: window.__mlsOutcomeEveryVisit.revert()) */
;(function(){try{var A="feat_mls_find_doctors.js";if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement("script");s.src=A+"?v=20260714fd2";s.setAttribute("data-mls-asset",A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* restore loader: Find Doctors button + provider picker (additive, reversible: window.__mlsFindDoctors) */
...
;(function(){try{var A="feat_mls_patient_snapshot.js";if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement("script");s.src=A+"?v=20260729listlinear";s.setAttribute("data-mls-asset",A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* restore loader: patient snapshot popover (window.__mlsPatientSnapshot) */
...
;(function(){try{var A="feat_mls_keyboard_layer.js";if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement("script");s.src=A+"?v=20260801kl1c2";s.setAttribute("data-mls-asset",A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* restore loader: unified keyboard layer (window.__mlsKbd) */
...
;(function(){try{var A="feat_mls_show_assistant.js";if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement("script");s.src=A+"?v=20260718sa3";s.setAttribute("data-mls-asset",A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* restore loader: show MLS assistant entry (window.__mlsShowAsst) */
...
;(function(){try{var A="feat_mls_staging_pack1.js";if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement("script");s.src=A+"?v=20260701sp1";s.setAttribute("data-mls-asset",A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* item80 (STAGING): pack1 -- Templates suite (upload files/folder w/ AI cleanup, starter pack, delete-all w/ backup+restore, std-line to bottom, always-add-line toggle, upload btn in op-prep), Simple-mode dictation tunnel (Who-Talk-Generate-Review-Finish, drives real controls, never signs), MLS Agent side chat dock (whitelisted actions only: views/pull/list-day/prep-op-notes/generate-all/find-patient/starters/tunnel/blanks; no writeback/sign/delete). Revert: window.__mlsPack1.revert() */
...
;(function(){try{
  ...
  if(old&&typeof old.revert==='function'){try{old.revert();}catch(e0){}}
  try{delete window.__mlsExactEncounterVerify;}catch(e1){window.__mlsExactEncounterVerify=null;}
  stale=document.querySelectorAll('script[data-mls-asset="'+A+'"]');
  for(i=0;i<stale.length;i++){node=stale[i];if(node&&node.parentNode)node.parentNode.removeChild(node);}
}catch(e){}})();
/* REJECTED_EEV_ROLLBACK_END */
;(function(){try{var A='feat_mls_staging_exact_actions.js';if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement('script');s.src=A+'?v=20260719sea100';s.setAttribute('data-mls-asset',A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* sea-1.0.0 STAGING: raw Visit engine actions require one exact scheduled appointment binding */
