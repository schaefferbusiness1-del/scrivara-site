...
;(function(){try{var A='feat_mls_selected_day_sync.js';if(document.querySelector('script[data-mls-asset="'+A+'"]'))return;var s=document.createElement('script');s.src=A+'?v=20260625sds1';s.setAttribute('data-mls-asset',A);s.async=false;(document.body||document.head||document.documentElement).appendChild(s);}catch(e){}})(); /* item29: unify the selected day - NEXT UP picker (.mlsnu-date) drives the Calendar (_calRefDate) so both surfaces show the same day; edge-triggered, never clobbers independent calendar navigation (additive, reversible: window.__mlsDaySync.revert()) */
