...
(function(){
  ...
  function mine(x){return x.closest&&(x.closest('#mlsCockpit')||x.closest('#mls-rt-bar')||x.closest('#mlsEwStack')||x.closest('#mlsWizBar'));}
  ...
  var STEPS=[null,
   {title:'Confirm your patient',hint:'Pick who you are seeing, then start the visit.',actions:[
   ...
   {title:'Capture the visit',hint:'Record or paste - when you stop, the note writes itself.',actions:[
   ...
   {title:'Review the note',hint:'Check the AI note and edit anything before it hits the chart.',actions:[
   ...
   {title:'Finalize & send',hint:'Push the note into athena, sign, and send the patient their portal.',actions:[
     {label:'\uD83D\uDD8A Sign & Save in Athena',primary:1,run:function(){go([{re:/Sign\s*&\s*Save in Athena/i}],'Generate the note first, then sign.');}},
     {label:'\uD83D\uDCE7 Send portal login',run:function(){go([{id:'mlsPortalInviteBtn'},{re:/Send portal login/i}],'Send-portal not found.');}},
     {label:'\uD83D\uDCC4 After-visit summary',run:function(){go([{id:'mlsavsBtn'},{re:/After.?visit summary/i}],'AVS not available yet.');}}]}
  ];
  ...
  function render(){var step=curStep();var spec=STEPS[step];if(!spec)return;var c=ck();var sh='';for(var i=1;i<=4;i++){var cl='ck-step'+(i===step?' on':(i<step?' done':''));var mk=(i<step)?'\u2713':i;sh+='<div class="'+cl+'" data-goto="'+i+'"><span class="n">'+mk+'</span>'+LB[i]+'</div>';}var ah='';spec.actions.forEach(function(a,idx){ah+='<button class="ck-btn'+(a.primary?' primary':'')+'" data-act="'+idx+'">'+esc(a.label)+'</button>';});c.innerHTML='<div class="ck-eyebrow">\u2728 MLS Easy \u00b7 guided visit</div><div class="ck-steps">'+sh+'</div><div class="ck-body"><div class="ck-title">'+esc(spec.title)+'</div><div class="ck-hint">'+esc(spec.hint)+'</div><div class="ck-actions">'+ah+'</div><div class="ck-nav"><button class="back"'+(step<=1?' disabled':'')+'>\u2190 Back</button><button class="next"'+(step>=4?' disabled':'')+'>'+(step>=4?'All set':'Next: '+LB[step+1]+' \u2192')+'</button></div></div>';[].forEach.call(c.querySelectorAll('[data-goto]'),function(el){el.onclick=function(){setStep(parseInt(el.getAttribute('data-goto'),10));};});[].forEach.call(c.querySelectorAll('[data-act]'),function(el){el.onclick=function(ev){try{ev.stopPropagation();ev.preventDefault();}catch(_){}spec.actions[parseInt(el.getAttribute('data-act'),10)].run();};});var bk=c.querySelector('.back');if(bk)bk.onclick=function(){setStep(step-1);};var nx=c.querySelector('.next');if(nx&&step<4)nx.onclick=function(){setStep(step+1);};}
  function mount(){var hero=document.getElementById('visitHero');if(!hero)return false;css();var c=ck();if(!c.parentNode){hero.insertBefore(c,hero.firstChild);}render();return true;}
